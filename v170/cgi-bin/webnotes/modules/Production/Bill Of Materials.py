class DocType:
  ...
  def get_item_detail(self, item_code):
    item = sql("select description from `tabItem` where is_active = 'Yes' and name = %s",item_code , as_dict =1)
    ret={
  ...
  def get_bom_material_detail(self, arg):
    ...
    if arg['item_code'] and arg['item_code'] == self.doc.item:
      ...
      raise Exception
    if arg['item_code']:
      item = sql("select is_asset_item, is_purchase_item, is_active, is_sub_contracted_item, description, stock_uom, default_bom from `tabItem` where item_code = %s", (arg['item_code']), as_dict = 1)
      ...
      if item and item[0]['is_active'] != 'Yes':
        msgprint("Item %s is NOT ACTIVE ITEM." % cstr(args['item_code']))
  ...
  def validate_main_item(self):
    item = sql("select is_manufactured_item, is_sub_contracted_item, is_active from `tabItem` where name = %s", self.doc.item, as_dict = 1)
    ...
    if item and item[0]['is_active'] != 'Yes':
      msgprint("Main Item %s is Not Active Item. Entered in BOM := %s" % (cstr(self.doc.item),cstr(self.doc.name)))
      raise Exception
  ...
  def validate_materials(self,m):
    ...
    item = sql("select is_manufactured_item, is_sub_contracted_item, is_active from`tabItem` where item_code = %s", m.item_code, as_dict = 1)
    if not item:
      msgprint("Item %s is not present in Item Master." % m.item_code)
      raise Exception
      
    if item[0]['is_active'] != 'Yes':
      msgprint("Item %s is NOT ACTIVE ITEM. " %  m.item_code)
      raise Exception
  ...
  def update_mar_lpr_sr(self, m, mar = 0, lpr = 0, sr = 0):
    m.moving_avg_rate, m.last_purchase_rate, m.standard_rate = self.get_mar_lpr_sr(cstr(m.item_code), mar, lpr, sr)
  ...
  def get_mar_lpr_sr(self, item_code, mar = 0, lpr = 0, sr = 0):
  ...
  # Check here this will be called from client - side
  #------------ FLAT BOM DETAIL --------------------------------------------------
  #---------------------------------------------------------------------------------
    ...
    for d in getlist(self.doclist, 'bom_materials'):
      ...
      if d.bom_no:
        ...
        if item and (item[0]['is_sub_contracted_item'] == 'Yes' or item[0]['is_pro_applicable'] == 'Yes'):
          # Put Docstatus =1 in where clause
          child_flat_bom_items = sql("select item_code, description, qty_consumed_per_unit, stock_uom, moving_avg_rate, last_purchase_rate, standard_rate, '%s' as parent_bom, bom_mat_no, 'No' as is_pro_applicable from `tabFlat BOM Detail` where parent = '%s' and is_pro_applicable = 'No'" % ( d.bom_no, cstr(d.bom_no)))
          ...
          # Put Docstatus =1 in where clause
          child_flat_bom_items = sql("select item_code, description, qty_consumed_per_unit, stock_uom, moving_avg_rate, last_purchase_rate, standard_rate, if(parent_bom = '%s', '%s', parent_bom) as parent_bom, bom_mat_no, is_pro_applicable from `tabFlat BOM Detail` where parent = '%s' " % ( d.bom_no, d.parent, cstr(d.bom_no)))
  ...
  def add_to_flat_bom_detail(self, is_submit = 0):
    ...
    for d in self.cur_flat_bom_items:
      fb_child = addchild( self.doc, 'flat_bom_details', 'Flat BOM Detail', 0, self.doclist)
      for i in fb_lbl:

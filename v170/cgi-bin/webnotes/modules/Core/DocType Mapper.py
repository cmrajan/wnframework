class DocType:
  ...
  def dt_map(self, from_doctype, to_doctype, from_docname, to_doc, doclist, from_to_list = '[]'):
    ...
    tbl_list = sql("select from_table, to_table, from_field, to_field, match_id, validation_logic from `%sTable Mapper Detail` where parent ='%s' order by match_id" % (self.prefix, from_doctype + "-" + to_doctype))
    ...
    for t in tbl_list:      
      ...
      if from_to in eval(from_to_list):
        fld_list = sql("select from_field, to_field from `%sField Mapper Detail` where parent = '%s' and match_id = %s and map = 'Yes'" % (self.prefix, from_doctype + "-" + to_doctype, match_id))
        if fld_list:   
          if not from_docname:

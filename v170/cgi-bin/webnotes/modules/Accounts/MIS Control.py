class DocType:
  def __init__(self, doc, doclist):
    ...
    if globals().has_key('version') and globals()['version'] == 'v170':
      self.roles = session['data']['profile']['roles']
    else:
  ...
  def get_comp(self):
    ...
    if has_common(self.roles, ['CRM Manager','Accounts Manager']):
      type.append('Sales')
    if has_common(self.roles, ['SRM Manager','Accounts Manager']):
  ...
  def get_trans_group(self,module):
    ...
    if module == 'Sales':
      ...
      group = ['Item','Item Group','Customer','Customer Group','Territory','Zone','Department','Chargeable']
    elif module == 'Purchase':
      st = ['Purchase Order','Purchase Receipt','Payable Voucher']
      group = ['Item','Item Group','Supplier','Supplier Type','Department']
  ...
  def get_totals(self, args):
    args = eval(args)
    totals = sql("SELECT %s FROM %s WHERE %s %s %s %s" %(args['query_val'], args['tables'], args['company'], args['cond'], args['add_cond'], args['fil_cond']), as_dict = 1)[0]
  ...
  def get_period_balance(self, acc, level, pl, company, fy):
    ...
    for p in self.period_list:
      ...
      bal = sql("select SUM(t1.debit), SUM(t1.credit) from `tabGL Entry` t1, `tabAccount` t2 WHERE t1.posting_date >= %s AND t1.posting_date <= %s AND t1.company = %s AND t1.account = t2.name AND t2.lft >= %s AND t2.rgt <= %s and ifnull(is_opening,'No') = 'No'", (sd,ed,company,lft,rgt))

class DocType:
  ...
  def check_budget(self,le_list,cancel):
    ...
    bgt = sql("select budget_allocated, actual, distribution_id from `tabBudget Detail` where account='%s' and parent='%s' and fiscal_year='%s'" % (acct,cost_center,cfy), as_dict =1)

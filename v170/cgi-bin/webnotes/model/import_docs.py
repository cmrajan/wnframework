					...
					try:
						...
						# Check Select Fields
							...
							if f!='naming_series':
								...
								exists = sql("select name from `tabNaming Series Options` where doc_type = %s", (self.dt_list[0]))
								if exists and cstr(exists[0][0]) and d[i] in cstr(exists[0][0]).split(NEWLINE):
									fd[f] = d[i]
								else:

...
_f.Frm.prototype.setup_std_layout=function(){this.tab_wrapper=$a(this.form_wrapper,'div');$dh(this.tab_wrapper);if(this.meta.section_style=='Tray'&&!get_url_arg('embed')){var t=$a(this.form_wrapper,'table','',{tableLayout:'fixed',width:'100%',borderCollapse:'collapse'});var r=t.insertRow(0);var c=r.insertCell(0);c.className='frm_tray_area';this.tray_area=c;this.body=$a(r.insertCell(1),'div','frm_body');}else{this.body=$a(this.form_wrapper,'div','frm_body');}
...
_f.Frm.prototype.setup_footer=function(){this.footer=$a(this.body,'div','green_buttons',{marginLeft:'8px'});var b=$a(this.footer,'button');b.innerHTML='Save';$(b).button({icons:{primary:'ui-icon-disk'}});b.onclick=function(){cur_frm.save('Save');}}
...
_f.Frm.prototype.refresh_footer=function(){if(this.editable&&!this.meta.in_dialog&&this.doc.docstatus==0&&!this.meta.istable&&this.get_doc_perms()[WRITE])
$ds(this.footer);else
$dh(this.footer);}
...
this.expand=function(){this.row.show();$y(this.header,{backgroundColor:'#AAA',fontWeight:'bold',color:'#FFF'});_f.cur_sec_header=this.header;if(me.df.label&&cur_frm.cscript[me.df.label]&&(!me.in_filter))
cur_frm.runclientscript(me.df.label,me.doctype,me.docname);}
this.make_row();this.make_simple_section(1,0);if(!isIE)this.collpase();}else{this.row=this.frm.layout.addsubrow();this.make_simple_section(1,1);}}else if(this.df){this.row=this.frm.layout.addrow();this.make_simple_section(1,1);}}
...
if(!this.button_area)this.button_area=$a(this.input_area,'span');this.input=$a(this.button_area,'button','',{width:'160px'});$y(this.input,{marginRight:'8px'});this.input.innerHTML=me.df.label.substr(0,16)+((me.df.label.length>15)?'..':'');this.input.onclick=function(){this.disabled=true;if(cur_frm.cscript[me.df.label]&&(!me.in_filter)){cur_frm.runclientscript(me.df.label,me.doctype,me.docname);this.disabled=false;}else

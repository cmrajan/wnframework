...
_f.FrmContainer.prototype.make_head=function(){var me=this;this.bg_color='#FFF';this.head=$a(this.wrapper,'div','',{margin:'0px 0px 4px 0px'});this.body=$a(this.wrapper,'div');var div=$a(this.head,'div','',{backgroundColor:this.bg_color});this.tbartab=make_table(div,1,2,'100%',['50%','50%'],{paddingTop:'2px'});$y($td(this.tbartab,0,0),{padding:'6px 4px 2px 4px'});this.main_title=$a($td(this.tbartab,0,0),'h1','',{margin:'0px'});this.sub_title=$a($td(this.tbartab,0,0),'div','',{display:'inline'});this.sub_title.is_inline=1;this.status_title=$a($td(this.tbartab,0,0),'span','',{marginLeft:'8px'});this.status_title.is_inline=1;this.tbar_div=$a($td(this.tbartab,0,1),'div','',{marginRight:'8px',textAlign:'right'})
...
_f.FrmContainer.prototype.make_toolbar=function(){this.btns={};var me=this;var makebtn=function(label,fn,bold,icon){var btn=$a(me.button_area,'button');btn.innerHTML=label;btn.onclick=fn;if(bold)$y(btn,{fontWeight:'bold'});btn.show=function(){if(isFF)$y(this,{display:'-moz-inline-box'});else $y(this,{display:'inline-block'});}
btn.hide=function(){$dh(this);}
me.btns[label]=btn;$(btn).button({icons:{primary:icon}});}
makebtn('Edit',function(){cur_frm.edit_doc(),0,'ui-icon-document'});makebtn('Save',function(){cur_frm.save('Save');},1,'ui-icon-disk');makebtn('Submit',function(){cur_frm.savesubmit();},0,'ui-icon-locked');makebtn('Cancel',function(){cur_frm.savecancel()},0,'ui-icon-closethick');makebtn('Amend',function(){cur_frm.amend_doc(),0,'ui-icon-scissors'});$(this.button_area).buttonset();}
...
var me=this;if(this.frm.meta.section_style=='Tabbed'){if(this.df.options!='Simple'){this.make_row();this.sec_id=this.frm.sections.length;this.frm.sections[this.sec_id]=this;this.frm.sections_by_label[me.df.label]=this;this.show=function(){if(me.df.label&&me.df.trigger=='Client'&&(!me.in_filter))
cur_frm.runclientscript(me.df.label,me.doctype,me.docname);}
this.mytab=this.frm.tabs.add_tab(me.df.label,this.show,this.row.wrapper);this.make_simple_section(1);}else{this.row=this.frm.layout.addsubrow();this.make_simple_section();}}else if(this.frm.meta.section_style=='Tray'){if(this.df.options!='Simple'){this.sec_id=this.frm.sections.length;this.frm.sections[this.sec_id]=this;this.frm.sections_by_label[me.df.label]=this;var w=$a(this.frm.tray_area,'div');this.header=$a(w,'div','',{padding:'4px 8px',cursor:'pointer'});this.header.innerHTML=me.df.label;this.header.onclick=function(){me.frm.set_section(me.sec_id);}

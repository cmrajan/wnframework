...
_f.FrmContainer.prototype.make_head=function(){var me=this;this.bg_color='#DDF';this.head=$a(this.wrapper,'div','',{borderBottom:'1px solid #AAA',margin:'0px'});this.body=$a(this.wrapper,'div');var div=$a(this.head,'div','',{backgroundColor:this.bg_color});$(div).corner('5px');this.tbartab=make_table(div,1,2,'100%',['50%','50%'],{paddingTop:'2px'});$y($td(this.tbartab,0,0),{padding:'6px 4px 2px 8px'});this.main_title=$a($td(this.tbartab,0,0),'div','',{fontFamily:'Helvetica',margin:'0px 8px 0px 0px',display:'inline',fontSize:'24px'});this.sub_title=$a($td(this.tbartab,0,0),'div','',{display:'inline'});this.sub_title.is_inline=1;this.status_title=$a($td(this.tbartab,0,0),'span','',{marginLeft:'8px'});this.status_title.is_inline=1;this.tbar_div=$a($td(this.tbartab,0,1),'div','',{marginRight:'8px',textAlign:'right'})

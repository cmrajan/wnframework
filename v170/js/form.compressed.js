...
_f.frm_dialog=null;_f.calling_doc_stack=[];_f.temp_access={};_f.FrmDialog=function(){var me=this;this.last_displayed=null;var d=new Dialog(640,400,'Edit Row');this.body=$a(d.body,'div','dialog_frm');$y(d.body,{backgroundColor:'#EEE'});d.done_btn_area=$a(d.body,'div','',{margin:'8px'});me.on_complete=function(){if(me.table_form){me.dialog.hide();}else{var callback=function(r){if(me.on_save_callback)
...
var f=new _f.Frm(doctype,parent);f.in_dialog=in_dialog;if(r.no_of_comments){f.n_comments[opt_name]=r.no_of_comments;f.comment_list[opt_name]=r.comment_list;}
...
if(opt_name&&!is_new){var args={'name':opt_name,'doctype':doctype,'getdoctype':1,'user':user};if(get_url_arg('akey'))args['akey']=get_url_arg('akey');if(from_archive)args['from_archive']=1;$c('webnotes.widgets.form.getdoc',args,callback);page_body.set_status('Loading Document...')}else{$c('webnotes.widgets.form.getdoctype',args={'doctype':doctype},callback);page_body.set_status('Loading Document...')}}
...
this.timestamp_area=$a(div,'span','link_type',{marginRight:'8px',fontSize:'11px'});this.timestamp_area.innerHTML='More Info';this.help_link=$a(div,'span','link_type',{marginRight:'8px',fontSize:'11px'});this.help_link.innerHTML='Help';this.status_area=$a(div,'span','',{marginRight:'8px',marginBottom:'2px',cursor:'pointer',textShadow:'none'})}
...
this.dn_area.innerHTML=f.docname;var doc=locals[f.doctype][f.docname];var sl=this.get_status_tags(doc,f);this.set_save_submit_color(doc);var t=this.status_area;t.innerHTML='';t.appendChild(sl[0]);if(sl[1])t.appendChild(sl[1]);this.timestamp_area.onclick=function(){msgprint(me.get_timestamp(doc))}
$dh(this.help_link)
if(locals.DocType[f.doctype].description){this.help_link.desc=locals.DocType[f.doctype].description;this.help_link.onclick=function(){msgprint(this.desc);}
$di(this.help_link);}}
...
_f.Frm.prototype.setup_std_layout=function(){this.wtab=make_table(this.form_wrapper,1,2,'100%',['75%','25%']);this.body=$td(this.wtab,0,0);this.meta.section_style='Simple';this.tab_wrapper=$a(this.body,'div');$dh(this.tab_wrapper);this.layout=new Layout(this.body,'100%');this.setup_sidebar();this.setup_footer();if(!(this.meta.istable||user=='Guest'))this.frm_head=new _f.FrmHeader(this.head,this);if(this.frm_head&&this.meta.in_dialog)$dh(this.frm_head.page_head.close_btn);this.setup_tips();if(this.meta.colour)
...
_f.Frm.prototype.setup_footer=function(){var me=this;this.footer=$a(this.form_wrapper,'div','',{display:'none'});var b=$btn(this.footer,'Save',function(){cur_frm.save('Save');},{marginLeft:'0px'},'green');this.footer.show_save=function(){$ds(me.footer);}
this.footer.hide_save=function(){$dh(me.footer);}}
...
if(user!='Guest'){$di(this.view_btn_wrapper);if(cur_frm.doc.__archived||(!this.orig_perm[0][WRITE])){$dh(this.view_btn_wrapper);}}else{$dh(this.view_btn_wrapper);$dh(this.print_close_btn);}
...
if(!this.meta.istable){this.refresh_header();this.sidebar.refresh();}
...
_f.Frm.prototype.refresh_footer=function(){if(this.footer){if(get_url_arg('embed')||(this.editable&&!this.meta.in_dialog&&this.doc.docstatus==0&&!this.meta.istable&&this.get_doc_perms()[WRITE])){this.footer.show_save();}else{this.footer.hide_save();}}}
...
if(r.n_comments)this.n_comments[me.docname]=r.no_of_comments;if(r.comment_list)this.comment_list[me.docname]=r.comment_list;me.runclientscript('setup',me.doctype,me.docname);me.refresh();}
...
_f.set_value=function(dt,dn,fn,v){var d=locals[dt][dn];var changed=d[fn]!=v;if(changed&&(d[fn]==null||v==null)&&(cstr(d[fn])==cstr(v)))changed=0;if(!d)
msgprint('error:Trying to set a value for "'+dt+','+dn+'" which is not found');if(changed){d[fn]=v;d.__unsaved=1;var frm=frms[d.doctype];try{if(d.parent&&d.parenttype){locals[d.parenttype][d.parent].__unsaved=1;frm=frms[d.parenttype];}}catch(e){if(d.parent&&d.parenttype)
...
this.cell=this.frm.layout.addcell(this.df.width);$y(this.cell.wrapper,{padding:'8px'});_f.cur_col_break_width=this.df.width;var fn=this.df.fieldname?this.df.fieldname:this.df.label;if(this.df&&this.df.label){this.label=$a(this.cell.wrapper,'h3');this.label.innerHTML=this.df.label;}}
...
_f.SectionBreak.prototype.make_collapsible=function(head){var me=this;var div=$a(head,'div','',{padding:'4px 4px',backgroundColor:'#EEE'});$gr(div,'#DDD','#CCC');$br(div,'3px');if(cur_frm.meta.in_dialog)$y(div,{marginLeft:'8px'});this.chk=$a_input(div,'checkbox',null,{marginRight:'8px'})
this.label=$a(div,'span','',{fontSize:'16px',fontFamily:'Georgia'});this.label.innerHTML=this.df.label?this.df.label:'';var d=this.df.description;if(d){this.desc_area=$a(div,'span','',{color:'#888',marginLeft:'8px'});this.desc_area.innerHTML=d.substr(0,80)+(d.length>80?'...':'');}
var span=$a(div,'span','link_type',{cssFloat:'right',marginRight:'8px'});span.innerHTML='Go To Top';span.onclick=function(){scroll(0,0);}
...
_f.SectionBreak.prototype.make_simple_section=function(with_header){var head=$a(this.row.main_head,'div','',{margin:'8px 8px 0px 0px'});this.row.header.head=head;var me=this;if(this.df.colour){var col=this.df.colour.split(':')[1];if(col!='FFF'){$y(this.row.sub_wrapper,{margin:'8px',padding:'0px',backgroundColor:('#'+col)});}}
if(with_header){if(this.df.label&&this.df.options!='Simple'){this.make_collapsible(head);}else{$y(head,{borderBottom:'1px solid #AAA',fontWeight:'bold',paddingBottom:'4px'});if(this.df.label){head.innerHTML=this.df.label}}}
...
if(this.df.fieldtype=='Text Editor'){$(me.input).tinymce({script_url:'js/tiny_mce_33/tiny_mce.js',theme:"advanced",plugins:"style,inlinepopups,table",extended_valid_elements:"div[id|dir|class|align|style]",width:'100%',height:'360px',theme_advanced_buttons1:"save,newdocument,|,bold,italic,underline,strikethrough,|,justifyleft,justifycenter,justifyright,justifyfull,|,styleselect,formatselect,fontselect,fontsizeselect",theme_advanced_buttons2:"cut,copy,paste,pastetext,pasteword,|,search,replace,|,bullist,numlist,|,outdent,indent,blockquote,|,undo,redo,|,link,unlink,cleanup,help,code,|,forecolor,backcolor,|,indicime,indicimehelp",theme_advanced_buttons3:"tablecontrols,styleprops,|,hr,removeformat,visualaid,|,sub,sup",theme_advanced_toolbar_location:"top",theme_advanced_toolbar_align:"left",content_css:"js/tiny_mce_33/custom_content.css",oninit:function(){me.init_editor();}});}else{$y(me.input,{fontFamily:'Courier, Fixed'});}}
...
_p.add_layout=function(dt,no_letterhead){var l=new Layout();if(locals['DocType'][dt].print_outline=='Yes')l.with_border=1;var cp=locals['Control Panel']['Control Panel'];l.addrow();if(!no_letterhead){if(cur_frm.doc.letter_head)
l.cur_row.header.innerHTML=_p.letter_heads[cur_frm.doc.letter_head];else if(cp.letter_head)
...
this.meta.max_attachments=10;var tab=$a($a(this.attach_area,'div'),'table');tab.insertRow(0);var label_area=tab.rows[0].insertCell(0);var main_area=tab.rows[0].insertCell(1);this.files_area=$a(main_area,'div');this.btn_area=$a(main_area,'div');$w(label_area,"33%");var d=$a(label_area,'div');$a(d,'span').innerHTML='<h4>File Attachments:</h4>';var btn_add_attach=$btn(this.btn_area,'+ Add');btn_add_attach.onclick=function(){me.add_attachment();me.sync_attachments(me.docname);me.refresh_attachments();}}
...
this.delbtn.onclick=this.remove;this.upload_div=$a(main_area,'div');this.download_div=$a(main_area,'div');this.uploader=new Uploader(this.upload_div,{from_form:1,doctype:frm.doctype,docname:frm.docname,at_id:at_id},_f.file_upload_done);this.download_link=$a(this.download_div,'a','link_type');this.refresh=function(){if(this.filename){$dh(this.upload_div);this.download_link.innerHTML=this.filename;this.download_link.href=outUrl+'?cmd=get_file&fname='+this.fileid;this.download_link.target="_blank";$ds(this.download_div);}else{$ds(this.upload_div);$dh(this.download_div);}}}
...
CommentList.prototype.add_comment=function(){var me=this;var args={};args.comment=this.input.value;args.comment_by=user;args.comment_by_fullname=user_fullname;args.comment_doctype=this.dt;args.comment_docname=this.dn;$c('webnotes.widgets.form.add_comment',args,function(r,rt){me.lst.run();cur_frm.n_comments[cur_frm.doc.name]=cint(cur_frm.n_comments[cur_frm.doc.name])+1;cur_frm.last_comments[cur_frm.doc.name]=me.input.value;me.input.value='';cur_frm.frm_head.refresh_comments();cur_frm.refresh_last_comment();});}
...
CommentItem.prototype.cmt_delete=function(cell,ri,ci,d){var me=this;if(d[ri][2]==user||d[ri][3]==user){del=$a(cell,'div','wn-icon ic-trash',{cursor:'pointer'});del.cmt_id=d[ri][0];del.onclick=function(){$c('webnotes.widgets.form.remove_comment',{id:this.cmt_id,dt:cur_frm.doctype,dn:cur_frm.docname},function(r,rt){cur_frm.no_of_comments=cur_frm.no_of_comments-1;cur_frm.frm_head.refresh_comments();me.comment.lst.run();});}}}
wn.widgets.form.sidebar={Sidebar:function(form){var me=this;this.form=form;this.opts={sections:[{title:'Actions',items:[{type:'link',label:'New',icon:'ic-doc_new',display:function(){return in_list(profile.can_create,form.doctype)},onclick:function(){new_doc(me.form.doctype)}},{type:'link',label:'Refresh',icon:'ic-playback_reload',onclick:function(){me.form.reload_doc()}},{type:'link',label:'Print',display:function(){return!me.form.doc.__islocal},icon:'ic-print',onclick:function(){me.form.print_doc()}},{type:'link',label:'Email',display:function(){return!me.form.doc.__islocal},icon:'ic-mail',onclick:function(){me.form.email_doc()}},{type:'link',label:'Copy',display:function(){return in_list(profile.can_create,me.form.doctype)&&!me.form.meta.allow_copy},icon:'ic-clipboard_copy',onclick:function(){me.form.copy_doc()}},{type:'link',label:'Trash',display:function(){return me.form.meta.allow_trash&&cint(me.form.doc.docstatus)!=2&&(!me.form.doc.__islocal)&&me.form.perm[0][CANCEL]},icon:'ic-trash',onclick:function(){me.form.savetrash()}}]},{title:'Tags',render:function(wrapper){me.form.taglist=new TagList(wrapper,me.form.doc._user_tags?me.form.doc._user_tags.split(','):[],me.form.doctype,me.form.docname,0,function(){});},display:function(){return!me.form.doc.__islocal}},{title:'Comments',render:function(wrapper){new wn.widgets.form.sidebar.SidebarComment(wrapper,me);},display:function(){return!me.form.doc.__islocal}}]}
this.refresh=function(){if(!this.sidebar){this.sidebar=new wn.widgets.PageSidebar($td(this.form.wtab,0,1),this.opts);}
this.sidebar.refresh();}},SidebarComment:function(parent,sidebar){var me=this;this.sidebar=sidebar;this.make_body=function(){if(this.wrapper)this.wrapper.innerHTML='';else this.wrapper=$a(parent,'div','sidebar-comment-wrapper');this.input=$a_input(this.wrapper,'text');this.btn=$btn(this.wrapper,'Post',function(){me.add_comment()},{marginLeft:'8px'});this.render_comments()}
this.render_comments=function(){var f=this.sidebar.form;var cl=f.comment_list?f.comment_list[f.docname]:null;this.msg=$a(this.wrapper,'div','sidebar-comment-message');if(cl){this.msg.innerHTML=cl.length+' out of '+f.n_comments[f.docname]+' comments';if(f.n_comments[f.docname]>cl.length){this.msg.innerHTML+=' <span class="link_type" onclick="cur_frm.show_comments()">Show all</span>'}
for(var i=0;i<cl.length;i++){this.render_one_comment(cl[i]);}}else{this.msg.innerHTML='Be the first one to comment.'}}
this.render_one_comment=function(det){$a(this.wrapper,'div','sidebar-comment-text','',det.comment);$a(this.wrapper,'div','sidebar-comment-info','',comment_when(det.creation)+' by '+det.comment_by_fullname);}
this.add_comment=function(){if(!this.input.value)return;this.btn.set_working();$c('webnotes.widgets.form.add_comment',this.get_comment_args(),function(r,rt){me.btn.done_working();var f=me.sidebar.form
me.update_comment_list();me.input.value='';me.make_body();});}
this.get_comment_args=function(){var f=this.sidebar.form
return{comment:this.input.value,comment_by:user,comment_by_fullname:user_fullname,comment_doctype:f.doctype,comment_docname:f.docname}}
this.update_comment_list=function(){var f=this.sidebar.form;f.n_comments[f.docname]=cint(f.n_comments[f.docname])+1;f.comment_list[f.docname]=add_lists([this.get_comment_args()],f.comment_list[f.docname]);}
this.make_body();}}
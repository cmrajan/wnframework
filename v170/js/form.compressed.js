...
_f.FrmContainer.prototype.make_head=function(){var me=this;this.bg_color='#DDF';this.head=$a(this.wrapper,'div','',{borderBottom:'1px solid #AAA',margin:'0px'});this.body=$a(this.wrapper,'div');var div=$a(this.head,'div','',{backgroundColor:this.bg_color});this.tbartab=make_table(div,1,2,'100%',['50%','50%'],{paddingTop:'2px'});$(div).corners('5px');$y($td(this.tbartab,0,0),{padding:'6px 4px 2px 8px'});this.main_title=$a($td(this.tbartab,0,0),'div','',{fontFamily:'Helvetica',margin:'0px 8px 0px 0px',display:'inline',fontSize:'24px'});this.sub_title=$a($td(this.tbartab,0,0),'div','',{display:'inline'});this.sub_title.is_inline=1;this.status_title=$a($td(this.tbartab,0,0),'span','',{marginLeft:'8px'});this.status_title.is_inline=1;this.tbar_div=$a($td(this.tbartab,0,1),'div','',{marginRight:'8px',textAlign:'right'})
...
_f.Grid.prototype.make_ui=function(parent){var ht=make_table($a(parent,'div'),1,2,'100%',['60%','40%']);this.main_title=$td(ht,0,0);this.main_title.className='columnHeading';$td(ht,0,1).style.textAlign='right';this.tbar_div=$a($td(ht,0,1),'div','grid_tbarlinks');$(this.tbar_div).corners('top-right top-left');this.tbar_tab=make_table(this.tbar_div,1,4,'100%',['25%','25%','25%','25%']);this.wrapper=$a(parent,'div','grid_wrapper');$h(this.wrapper,cint(screen.width*0.5)+'px');this.head_wrapper=$a(this.wrapper,'div','grid_head_wrapper');this.head_tab=$a(this.head_wrapper,'table','grid_head_table');this.head_row=this.head_tab.insertRow(0);this.tab_wrapper=$a(this.wrapper,'div','grid_tab_wrapper');this.tab=$a(this.tab_wrapper,'table','grid_table');var me=this;this.wrapper.onscroll=function(){me.head_wrapper.style.top=me.wrapper.scrollTop+'px';}}

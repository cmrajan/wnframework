...
_f.FrmContainer.prototype.make_head=function(){var me=this;this.bg_color='#DDF';this.head=$a(this.wrapper,'div','',{borderBottom:'1px solid #AAA',margin:'0px'});this.body=$a(this.wrapper,'div');var div=$a(this.head,'div','',{backgroundColor:this.bg_color});this.tbartab=make_table(div,1,2,'100%',['50%','50%'],{paddingTop:'2px'});$y($td(this.tbartab,0,0),{padding:'6px 4px 2px 8px'});this.main_title=$a($td(this.tbartab,0,0),'div','',{fontFamily:'Helvetica',margin:'0px 8px 0px 0px',display:'inline',fontSize:'24px'});this.sub_title=$a($td(this.tbartab,0,0),'div','',{display:'inline'});this.sub_title.is_inline=1;this.status_title=$a($td(this.tbartab,0,0),'span','',{marginLeft:'8px'});this.status_title.is_inline=1;this.tbar_div=$a($td(this.tbartab,0,1),'div','',{marginRight:'8px',textAlign:'right'})
var tab2=make_table(this.tbar_div,1,4,'220px',['160px','60px'],{textAlign:'center',padding:'3px',verticalAlign:'middle'});$y(tab2,{cssFloat:'right'});this.tbarlinks=new SelectWidget($td(tab2,0,0),[]);$y($td(tab2,0,1),{padding:'6px 0px 2px 0px',textAlign:'right'});this.close_btn=$a($td(tab2,0,1),'img','',{cursor:'pointer'});this.close_btn.src="images/icons/close.gif";this.close_btn.onclick=function(){nav_obj.show_last_open();}

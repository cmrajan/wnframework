...
_f.FrmContainer=function(){this.wrapper=page_body.add_page("Forms",function(){});this.make_head();this.make_toolbar();}
_f.FrmContainer.prototype.make_head=function(){var me=this;this.bg_color='#DDF';this.head=$a(this.wrapper,'div','',{borderBottom:'1px solid #AAA',margin:'0px'});this.body=$a(this.wrapper,'div');this.tbartab=make_table($a(this.head,'div'),1,2,'100%',['50%','50%'],{backgroundColor:this.bg_color,borderBottom:'1px solid #AAF'});$y($td(this.tbartab,0,0),{padding:'4px'});this.main_title=$a($td(this.tbartab,0,0),'h2','',{margin:'0px 8px',display:'inline'});this.sub_title=$a($td(this.tbartab,0,0),'div','',{display:'inline'});this.sub_title.is_inline=1;this.status_title=$a($td(this.tbartab,0,0),'span','',{marginLeft:'8px'});this.status_title.is_inline=1;this.tbar_div=$a($td(this.tbartab,0,1),'div','',{marginRight:'8px',textAlign:'right'})
...
this.comments_btn.onmouseover=function(){$y(c,{backgroundColor:'#EEF'});if(me.doc.__islocal){return;}
...
me.btns[label]=btn;}
makebtn('Edit',function(){cur_frm.edit_doc()});makebtn('Save',function(){cur_frm.save('Save');},1);makebtn('Submit',function(){cur_frm.savesubmit();});makebtn('Cancel',function(){cur_frm.savecancel()});makebtn('Amend',function(){cur_frm.amend_doc()});me.tbarlinks.onchange=function(){var v=sel_val(this);if(v=='New')new_doc();else if(v=='Refresh')cur_frm.reload_doc();else if(v=='Print')cur_frm.print_doc();else if(v=='Email')cur_frm.email_doc();else if(v=='Copy')cur_frm.copy_doc();}}
...
if(this.docname==old)
this.docname=name;if(this&&this.opendocs[doc.localname]){local_dt[doc.doctype][doc.name]=local_dt[doc.doctype][doc.localname];local_dt[doc.doctype][doc.localname]=null;}
...
_f.Frm.prototype.email_doc=function(){sel=this.print_sel;var c=$td(_e.dialog.rows['Format'].tab,0,1);if(c.cur_sel)c.removeChild(c.cur_sel);c.appendChild(this.print_sel.wrapper);c.cur_sel=this.print_sel.wrapper;_e.dialog.widgets['Subject'].value=this.meta.name+': '+this.docname;_e.dialog.show();}
...
_f.Frm.prototype.set_section=function(sec_id){this.sections[this.cur_section[this.docname]].hide();this.sections[sec_id].show();this.cur_section[this.docname]=sec_id;}
...
_f.Frm.prototype.setup_fields_std=function(){var fl=fields_list[this.doctype];if(fl[0]&&fl[0].fieldtype!="Section Break"){this.layout.addrow();if(fl[0].fieldtype!="Column Break")
this.layout.addcell();}
...
this.refresh_tabs();this.refresh_fields();this.refresh_dependency();if(this.layout)this.layout.show();if(this.meta.allow_attach)this.refresh_attachments();if(!this.display)this.show(this.docname,1);}}
...
_f.Frm.prototype.check_required=function(dt,dn){var doc=locals[dt][dn];if(doc.docstatus>1)return true;var fl=fields_list[dt];if(!fl)return true;var all_clear=true;var errfld=[];for(var i=0;i<fl.length;i++){var key=fl[i].fieldname;var v=doc[key];if(fl[i].reqd&&is_null(v)){errfld[errfld.length]=fl[i].label;if(all_clear)all_clear=false;}}
if(errfld.length)msgprint('<b>Following fields are required:</b>\n'+errfld.join('\n'));return all_clear;}
_f.Frm.prototype.savedoc=function(dt,dn,save_action,onsave,onerr){var doc=locals[dt][dn];var doctype=locals['DocType'][dt];var tmplist=[];var doclist=make_doclist(dt,dn,1);var all_clear=true;if(save_action!='Cancel'){for(var n in doclist){var tmp=check_required(doclist[n].doctype,doclist[n].name);if(doclist[n].docstatus+''!='2'&&all_clear)
all_clear=tmp;}}
...
_f.Frm.prototype.savesubmit=function(){var answer=confirm("Permanently Submit "+cur_frm.docname+"?");if(answer)this.save_doc('Submit');}
_f.Frm.prototype.savecancel=function(){var answer=confirm("Permanently Cancel "+cur_frm.docname+"?");if(answer)this.save_doc('Cancel');}
...
this.cell=this.frm.layout.addcell(this.df.width);_f.cur_col_break_width=this.df.width;var fn=this.df.fieldname?this.df.fieldname:this.df.label;if(this.df&&this.df.label){this.label=$a(this.cell.wrapper,'div','columnHeading');this.label.innerHTML=this.df.label;}}
...
_f.cur_sec_header=null;_f.SectionBreak.prototype.make_body=function(){this.fields=[];if((!this.perm[this.df.permlevel])||(!this.perm[this.df.permlevel][READ])||this.df.hidden){return;}
...
_f.Grid.prototype.show=function(){$ds(this.wrapper);if(this.can_add_rows){if(this.is_scrolltype)$ds(this.tbar_div);else $ds(this.btn_area);}else{if(this.is_scrolltype)$dh(this.tbar_div);else $dh(this.btn_area);}}
...
function grid_click_event(e,target){if(_f.cur_grid_cell&&!target.isactive){if(!text_dialog.display&&!selector.display){_f.cur_grid_cell.grid.cell_deselect();}}}
_f.Grid.prototype.cell_deselect=function(){if(_f.cur_grid_cell){var c=_f.cur_grid_cell;c.grid.remove_template(c);c.div.className='grid_cell_div';if(c.is_odd)c.div.style.border='2px solid '+c.grid.alt_row_bg;else c.div.style.border='2px solid #FFF';_f.cur_grid_cell=null;_f.cur_grid=null;this.isactive=false;}}
...
hc.template.perm=this.field?this.field.perm:hc.perm;if(hc.fieldname&&hc.template.get_status()=='Write'){this.cell_deselect();cell.div.style.border='2px solid #88F';_f.cur_grid_cell=cell;this.add_template(cell);this.isactive=true;}}
_f.Grid.prototype.add_template=function(cell){if(!cell.row.docname&&this.add_newrow){this.add_newrow();this.cell_select(cell);}else{var hc=this.head_row.cells[cell.cellIndex];cell.div.innerHTML='';cell.div.appendChild(hc.template.wrapper);hc.template.activate(cell.row.docname);hc.template.activated=1;}}
...
_f.grid_refresh_field=function(temp,input){if(input.value!=get_value(temp.doctype,temp.docname,temp.df.fieldname))
...
_p.build_dialog=function(){if(!_p.dialog){_p.make_dialog();}
_p.dialog.show();}
_p.make_dialog=function(){var d=new Dialog(360,140,"Print Formats");$dh(d.wrapper);d.make_body([['HTML','Select'],['Button','Go',function(){_p.build(sel_val(cur_frm.print_sel),go);}]]);_p.print_doc_dialog=d;d.onshow=function(){var c=d.widgets['Select'];if(c.cur_sel)c.removeChild(c.cur_sel);c.appendChild(cur_frm.print_sel.wrapper);c.cur_sel=cur_frm.print_sel.wrapper;}}
...
+".pagesubhead { font-size: 12px; font-weight: bold; font-family: verdana; padding: 2px 10px 10px 0px; }";_p.def_print_style="html, body{ font-family: Arial, Helvetica; font-size: 12px; }"
+"\nbody { margin: 12px; }"
+"td {padding: 2px;}"
...
var doc=locals[cur_frm.doctype][cur_frm.docname];if(fmtname=='Standard'){onload(_p.render(print_std(),_p.print_style,doc,doc.name));}else{if(!_p.formats[fmtname])
$c('get_print_format',{'name':fmtname},function(r,t){_p.formats[fmtname]=r.message;onload(_p.render(_p.formats[fmtname],'',doc,doc.name));});else
...
$c('get_contact_list',{'select':_e.email_as_field,'from':_e.email_as_dt,'where':_e.email_as_in,'txt':(last_txt?strip(last_txt):'%')},call_back);return;}
var sel;_e.dialog=d;}

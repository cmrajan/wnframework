...
_f.FrmContainer=function(){this.wrapper=page_body.add_page("Forms",function(){});$dh(this.wrapper);$y(this.wrapper,{margin:'4px'});this.make_head();this.make_toolbar();}
_f.FrmContainer.prototype.make_head=function(){var me=this;this.bg_color='#FFF';this.head=$a(this.wrapper,'div','',{margin:'0px 0px 4px 0px'});this.body=$a(this.wrapper,'div');var div=$a(this.head,'div','');this.tbartab=make_table(div,1,3,'100%',['35%','20%','45%'],{paddingTop:'2px'});$y(this.tbartab,{tableLayout:'fixed'});$y($td(this.tbartab,0,0),{padding:'4px'});this.main_title=$a($td(this.tbartab,0,0),'h1','',{margin:'0px'});this.button_area=$a($td(this.tbartab,0,1),'div','');this.tbar_div=$a($td(this.tbartab,0,2),'div','',{marginRight:'8px',textAlign:'right'})
this.tbartab2=make_table($a(this.head,'div'),1,2,'100%',['50%','50%']);this.owner_img=$a($td(this.tbartab2,0,0),'img','',{margin:'4px',width:'40px',display:'inline'});this.owner_img.is_inline=1;this.mod_img=$a($td(this.tbartab2,0,0),'img','',{margin:'4px',width:'40px',display:'inline'});this.mod_img.is_inline=1;$y($td(this.tbartab2,0,1),{textAlign:'right',paddingRight:'12px'});this.sub_title=$a($td(this.tbartab2,0,1),'div','',{display:'inline',color:'#888'});this.sub_title.is_inline=1;this.status_title=$a($td(this.tbartab2,0,1),'span','',{marginLeft:'8px'});this.status_title.is_inline=1;this.last_update_area=$a($td(this.tbartab2,0,1),'span','',{marginLeft:'8px',color:'#888'});this.last_update_area.is_inline=1;this.head_elements=[this.button_area,this.tbar_div,this.owner_img,this.mod_img,this.sub_title,this.status_title,this.last_update_area];}
...
_f.FrmContainer.prototype.refresh_save_btns=function(){var me=this;var frm=cur_frm;var p=frm.get_doc_perms();me.button_area.innerHTML='';var makebtn=function(label,fn,bold,icon){var btn=$a(me.button_area,'button');btn.innerHTML=label;btn.onclick=fn;if(bold)$y(btn,{fontWeight:'bold'});$(btn).button({icons:{primary:icon}});}
if(!cur_frm.editable)
makebtn('Edit',function(){cur_frm.edit_doc(),0,'ui-icon-document'});if(cur_frm.editable&&cint(frm.doc.docstatus)==0&&p[WRITE])
...
if(locals['DocType']&&locals['DocType'][doctype]){callback();}else if(opt_name&&(!LocalDB.is_doc_loaded(doctype,opt_name))){$c('webnotes.widgets.form.getdoc',{'name':opt_name,'doctype':doctype,'getdoctype':1,'user':user},callback);}else{$c('webnotes.widgets.form.getdoctype',args={'doctype':doctype},callback);}}
_f.frm_dialog=null;_f.edit_record=function(dt,dn,from_grid){if(!_f.frm_dialog){var d=new Dialog(640,400,'Edit Row');d.body_wrapper=$a(d.body,'div','dialog_frm');d.done_btn=$a($a(d.body,'div','',{margin:'8px'}),'button');d.done_btn.innerHTML='Done';d.done_btn.onclick=function(){if(!_f.frm_dialog.from_grid){var callback=function(r,rt){if(cur_frm&&calling_link_field){locals[cur_frm.doctype][cur_frm.docname][calling_link_field]=_f.frm_dialog.cur_frm.docname;refresh_field(calling_link_field);}
_f.frm_dialog.hide();}
_f.frm_dialog.cur_frm.save('Save',callback);}else{_f.frm_dialog.hide();}}
$(d.done_btn).button({icons:{primary:'ui-icon-disk'}});d.onhide=function(){if(_f.cur_grid)
_f.cur_grid.refresh_row(_f.cur_grid_ridx,_f.frm_dialog.dn);}
_f.frm_dialog=d;}
var d=_f.frm_dialog;var show_dialog=function(){var f=frms[dt];if(from_grid){f.parent_doctype=cur_frm.doctype;f.parent_docname=cur_frm.docname;}
f.meta.section_style='Simple';if(d.cur_frm)
{d.cur_frm.hide();}
f.show(dn,null,d.body_wrapper,1);d.cur_frm=frm;d.dn=dn;d.from_grid=from_grid;if(from_grid)
d.set_title("Editing Row #"+(_f.cur_grid_ridx+1));else
d.set_title(dt+': '+dn);d.show();}
if(!frms[dt]){_f.add_frm(dt,show_dialog,null,d.body_wrapper);}else{show_dialog();}}
_f.Frm=function(doctype,parent){this.docname='';this.doctype=doctype;this.display=0;this.in_dialog=false;var me=this;this.is_editable={};this.opendocs={};this.cur_section={};this.sections=[];this.sections_by_label={};this.grids=[];this.cscript={};this.parent=parent;this.attachments={};this.last_comments={};this.n_comments={};frms[doctype]=this;this.setup_meta(doctype);rename_observers.push(this);}
...
_f.Frm.prototype.setup_tips=function(){var me=this;this.tip_box=$a(this.tip_wrapper,'div','frm_tip_box');if(ui_effects)$(this.tip_box).corners();var tab=$a(this.tip_box,'table');var r=tab.insertRow(0);var c0=r.insertCell(0);this.c1=r.insertCell(1);this.img=$a(c0,'img');this.img.setAttribute('src','images/icons/lightbulb.gif');c0.style.width='24px';this.set_tip=function(t,icon){me.c1.innerHTML=t;$(me.tip_box).fadeIn();if(icon)this.img.setAttribute('src','images/icons/'+icon);}
...
if(this.in_dialog)this.meta.hide_heading=1;this.layout=new Layout(this.body,'100%');this.setup_tips();if(this.meta.section_style=='Tabbed')
...
_f.Frm.prototype.setup_template_layout=function(){this.body=$a(this.wrapper,'div');this.layout=null;this.body.innerHTML=this.meta.dt_template;var dt=this.doctype.replace(/ /g,'');this.meta.hide_heading=1;var fl=fields_list[this.doctype];for(var i=0;i<fl.length;i++){var f=fl[i];var fn=f.fieldname?f.fieldname:f.label;var field_area=$i('frm_'+dt+'_'+fn);if(field_area){var fld=make_field(f,this.doctype,field_area,this,0,1);this.fields[this.fields.length]=fld;this.fields_dict[fn]=fld;}}}
...
this.script_setup=1;}
_f.Frm.prototype.set_parent=function(parent){if(parent){if(this.wrapper)
parent.appendChild(this.wrapper);else
this.parent=parent;}}
_f.Frm.prototype.setup=function(){var me=this;this.fields=[];this.fields_dict={};this.wrapper=$a(this.parent,'div','frm_wrapper');if(this.meta.use_template){this.setup_template_layout();}else{this.setup_std_layout();}
...
_f.Frm.prototype.show=function(docname,from_refresh,parent,in_dialog){this.in_dialog=in_dialog;if(!this.in_dialog&&cur_frm&&cur_frm!=this){this.defocus_rest();cur_frm.hide();}
if(docname)
this.docname=docname;if(parent)
this.set_parent(parent);$ds(this.wrapper);this.display=1;if(!this.in_dialog)
...
_f.Frm.prototype.refresh_tabs=function(){var me=this;if(me.meta.section_style=='Tray'){for(var i in me.sections){me.sections[i].hide();}
...
var ret_fn=function(r){if(!cur_frm.in_dialog)
...
this.exp_icon.expand=function(){if(ui_effects)$ds(me.row.body)
else $(me.row.body).slideDown();me.exp_icon.src=min_icon;}
this.exp_icon.collapse=function(){if(ui_effects)$dh(me.row.body)
else $(me.row.body).slideUp();me.exp_icon.src=exp_icon;}
...
_f.SectionBreak.prototype.make_simple_section=function(static){var head=$a(this.row.header,'div','',{margin:'4px 8px 0px 8px'});var me=this;var has_col=false;if(this.df.colour){has_col=true;var col=this.df.colour.split(':')[1];if(col!='FFF'){$y(this.row.sub_wrapper,{margin:'8px',padding:'0px',backgroundColor:('#'+col)});$(this.row.sub_wrapper).corners();}}
...
if(this.df.label){this.make_collapsible(head);}else if(!has_col){$y(head,{margin:'8px',borderBottom:'2px solid #445'});}}
...
var me=this;if(this.frm.meta.section_style=='Tabbed'){if(this.df.options!='Simple'){this.make_row();this.sec_id=this.frm.sections.length;this.frm.sections[this.sec_id]=this;this.frm.sections_by_label[me.df.label]=this;this.onshow=function(){if(me.df.label&&me.df.trigger=='Client'&&(!me.in_filter))
cur_frm.runclientscript(me.df.label,me.doctype,me.docname);}
this.show=function(){me.frm.tabs.tabs[me.df.label].show();}
this.hide=function(){}
this.mytab=this.frm.tabs.add_tab(me.df.label,this.onshow,this.row.wrapper);this.make_simple_section(1);}else{this.row=this.frm.layout.addsubrow();this.make_simple_section();}}else if(this.frm.meta.section_style=='Tray'){if(this.df.options!='Simple'){this.sec_id=this.frm.sections.length;this.frm.sections[this.sec_id]=this;this.frm.sections_by_label[me.df.label]=this;var w=$a(this.frm.tray_area,'div');this.header=$a(w,'div','',{padding:'4px 8px',cursor:'pointer'});this.header.innerHTML=me.df.label;this.header.onclick=function(){me.frm.set_section(me.sec_id);}
$(this.header).hover(function(){if(_f.cur_sec_header!=this){$y(this,{backgroundColor:'#DDD'});}},function(){if(_f.cur_sec_header!=this){$y(this,{backgroundColor:'#FFF'});}});if(ui_effects)$(this.header).corners('top-left bottom-left');this.hide=function(){this.row.hide();$y(this.header,{backgroundColor:'#FFF',fontWeight:'normal',color:'#000'});}
...
_f.Grid.prototype.make_ui=function(parent){var ht=make_table($a(parent,'div'),1,2,'100%',['60%','40%']);this.main_title=$td(ht,0,0);this.main_title.className='columnHeading';$td(ht,0,1).style.textAlign='right';this.tbar_div=$a($td(ht,0,1),'div','grid_tbarlinks');if(isIE)$y(this.tbar_div,{width:'200px'});if(!isIE)$(this.tbar_div).corners('top-right top-left');this.tbar_tab=make_table(this.tbar_div,1,4,'100%',['25%','25%','25%','25%']);this.wrapper=$a(parent,'div','grid_wrapper');$h(this.wrapper,cint(screen.width*0.5)+'px');this.head_wrapper=$a(this.wrapper,'div','grid_head_wrapper');this.head_tab=$a(this.head_wrapper,'table','grid_head_table');this.head_row=this.head_tab.insertRow(0);this.tab_wrapper=$a(this.wrapper,'div','grid_tab_wrapper');this.tab=$a(this.tab_wrapper,'table','grid_table');var me=this;this.wrapper.onscroll=function(){me.head_wrapper.style.top=me.wrapper.scrollTop+'px';}}
...
this.btns[i].isactive=true;}else{this.btn_area.onclick=function(){me.make_newrow(1);var dn=me.add_newrow();cur_grid=me;cur_grid_ridx=me.tab.rows.length-1;edit_record(me.doctype,dn,1);}}}
...
_p.dialog.show();}
_p.make_dialog=function(){var d=new Dialog(360,140,"Print Formats");d.make_body([['HTML','Select'],['Button','Go',function(){_p.build(sel_val(cur_frm.print_sel),_p.go);}]]);_p.dialog=d;d.onshow=function(){var c=d.widgets['Select'];if(c.cur_sel)c.removeChild(c.cur_sel);c.appendChild(cur_frm.print_sel.wrapper);c.cur_sel=cur_frm.print_sel.wrapper;}}

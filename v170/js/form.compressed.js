...
this.timestamp_area=$a(div,'span','',{marginRight:'8px',cursfontWeight:'normal',cursor:'pointer',color:'#00B',fontSize:'11px',fontWeight:'normal',textDecoration:'underline',textShadow:'none'});this.timestamp_area.innerHTML='more info';this.status_area=$a(div,'span','',{marginRight:'8px',marginBottom:'2px',cursor:'pointer',textShadow:'none'})}
...
_f.Frm.prototype.refresh_footer=function(){if(get_url_arg('embed')||(this.editable&&!this.meta.in_dialog&&this.doc.docstatus==0&&!this.meta.istable&&this.get_doc_perms()[WRITE]&&this.footer)){this.footer.show_save();}else{this.footer.hide_save();}}
...
_f.SectionBreak.prototype.make_collapsible=function(head){var me=this;var div=$a(head,'div','',{padding:'4px 4px',backgroundColor:'#EEE'});$gr(div,'#DDD','#CCC');$br(div,'3px');this.chk=$a_input(div,'checkbox',null,{marginRight:'8px'})
...
if(this.df.fieldtype=='Text Editor'){$(me.input).tinymce({script_url:'js/tiny_mce_33/tiny_mce.js',theme:"advanced",plugins:"style,table",extended_valid_elements:"div[id|dir|class|align|style]",width:'100%',height:'360px',theme_advanced_buttons1:"save,newdocument,|,bold,italic,underline,strikethrough,|,justifyleft,justifycenter,justifyright,justifyfull,|,styleselect,formatselect,fontselect,fontsizeselect",theme_advanced_buttons2:"cut,copy,paste,pastetext,pasteword,|,search,replace,|,bullist,numlist,|,outdent,indent,blockquote,|,undo,redo,|,link,unlink,cleanup,help,code,|,forecolor,backcolor,|,indicime,indicimehelp",theme_advanced_buttons3:"tablecontrols,styleprops,|,hr,removeformat,visualaid,|,sub,sup",theme_advanced_toolbar_location:"top",theme_advanced_toolbar_align:"left",content_css:"js/tiny_mce_33/custom_content.css",oninit:function(){me.init_editor();}});}else{$y(me.input,{fontFamily:'Courier, Fixed'});}}
_f.CodeField.prototype.init_editor=function(){var me=this;this.editor=tinymce.get(this.myid);this.editor.onKeyUp.add(function(ed,e){me.set(ed.getContent());});this.editor.onPaste.add(function(ed,e){me.set(ed.getContent());});if(cur_frm)this.editor.setContent(locals[cur_frm.doctype][cur_frm.docname][this.df.fieldname]);}
...
CommentList.prototype.add_comment=function(){var me=this;var args={};args.comment=this.input.value;args.comment_by=user;args.comment_by_fullname=user_fullname;args.comment_doctype=this.dt;args.comment_docname=this.dn;$c_obj('Widget Control','add_comment',docstring(args),function(r,rt){me.lst.run();cur_frm.n_comments[cur_frm.doc.name]=cint(cur_frm.n_comments[cur_frm.doc.name])+1;cur_frm.last_comments[cur_frm.doc.name]=me.input.value;me.input.value='';cur_frm.frm_head.refresh_comments();cur_frm.refresh_last_comment();});}
...
CommentItem.prototype.cmt_delete=function(cell,ri,ci,d){var me=this;if(d[ri][2]==user||d[ri][3]==user){del=$a(cell,'div','wn-icon ic-trash',{cursor:'pointer'});del.cmt_id=d[ri][0];del.onclick=function(){$c_obj('Widget Control','remove_comment',JSON.stringify({id:this.cmt_id,dt:cur_frm.doctype,dn:cur_frm.docname}),function(r,rt){cur_frm.no_of_comments=cur_frm.no_of_comments-1;cur_frm.frm_head.refresh_comments();me.comment.lst.run();});}}}
...
this.set_sort_options();this.main_title.innerHTML=this.doctype;this.clear_graph();this.current_loaded=null;this.customized_filters=null;this.sc=null;this.has_index=1;this.has_headings=1;for(var i in this.fn_list)this[this.fn_list[i]]=null;this.refresh_filters();}
...
_r.ReportBuilder.prototype.set_criteria_sel=function(criteria_name){this.main_title.innerHTML=criteria_name;var sc=locals['Search Criteria'][this.sc_dict[criteria_name]];if(sc&&sc.add_col)
...
if(!hide_toolbar)this.make_toolbar(parent);this.wrapper=$a(parent,'div','report_tab');$h(this.wrapper,cint(screen.width*0.4)+'px');this.wrapper.onscroll=function(){scroll_head(this);}
...
_r.DataTable.prototype.add_sort_option=function(label,val){if(!this.sort_labels[this.dt])this.sort_labels[this.dt]={};this.sort_labels[this.dt][label]=val;var s=this.sort_sel;s.append(label);if(!s.inp.value)s.inp.value=label;}
...
_r.DataTable.prototype.do_export=function(){this._get_query(true);var me=this;export_ask_for_max_rows(this.query,function(q){export_csv(q,(me.rep_name?me.rep_name:me.dt),(me.search_criteria?me.search_criteria.name:''),me.is_simple,docstring(me.filter_vals));});}
_r.DataTable.prototype.do_calc=function(){show_calc(this.tab,this.colnames,this.coltypes,1);}

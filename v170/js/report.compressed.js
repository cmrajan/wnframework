...
_r.ReportContainer=function(){this.wrapper=page_body.add_page("Report Builder",function(){});$dh(this.wrapper);$y(this.wrapper,{margin:'4px'});var me=this;this.rb_dict={};var div=$a(this.wrapper,'div');var inner=$a(div,'div','',{padding:'6px 8px 4px 8px'});var htab=make_table(inner,1,2,'100%',['80%','20%']);this.main_title=$a($td(htab,0,0),'h1');$y($td(htab,0,1),{textAlign:'right'});this.close_btn=$a($a($td(htab,0,1),'div','',{padding:'2px',margin:'0px'}),'img','',{cursor:'pointer'});this.close_btn.src="images/icons/close.gif";this.close_btn.onclick=function(){nav_obj.show_last_open();}
...
var runbtn=$a(this.button_area,'button','',{fontSize:'14px'});runbtn.innerHTML='Run'.bold();runbtn.onclick=function(){if(me.cur_rb){me.cur_rb.dt.start_rec=1;me.cur_rb.dt.run();}}
$(runbtn).button({icons:{primary:'ui-icon-circle-triangle-e'}});$dh(this.button_area);this.rb_area=$a(this.wrapper,'div');this.set_dt=function(dt,onload){$dh(me.home_area);$ds(me.rb_area);$ds(me.button_area);my_onload=function(f){if(!f.forbidden){me.cur_rb=f;me.cur_rb.mytabs.tabs['Result'].show();if(onload)onload(f);}}
if(me.cur_rb)
...
_r.ReportBuilder.prototype.make_tabs=function(){this.tab_wrapper=$a(this.wrapper,'div','finder_tab_area');this.mytabs=new TabbedPage(this.tab_wrapper);this.mytabs.add_tab('Result');this.mytabs.add_tab('More Filters');this.mytabs.add_tab('Select Columns');}
_r.ReportBuilder.prototype.make_body=function(){_r.rb_con.main_title.innerHTML=this.doctype;var me=this;this.make_save_criteria();this.column_picker=new _r.ReportColumnPicker(this);this.report_filters=new _r.ReportFilters(this);}
...
this.report_filters.refresh();this.column_picker.clear();var cl=this.sc.columns.split(',');for(var c=0;c<cl.length;c++){var key=cl[c].split('\1');this.select_column(key[0],key[1],1);}
...
this.current_loaded=criteria_name;_r.rb_con.main_title.innerHTML=criteria_name;}
...
return 0;}
var me=this;me.make_body();var dt=me.parent_dt?me.parent_dt:me.doctype;var fl=[{'fieldtype':'Data','label':'ID','fieldname':'name','search_index':1,'parent':dt},{'fieldtype':'Data','label':'Owner','fieldname':'owner','search_index':1,'parent':dt},{'fieldtype':'Date','label':'Created on','fieldname':'creation','search_index':0,'parent':dt},{'fieldtype':'Date','label':'Last modified on','fieldname':'modified','search_index':0,'parent':dt},];if(can_dt_be_submitted(dt)){fl[fl.length]={'fieldtype':'Check','label':'Saved','fieldname':'docstatus','search_index':1,'def_filter':1,'parent':dt};fl[fl.length]={'fieldtype':'Check','label':'Submitted','fieldname':'docstatus','search_index':1,'def_filter':1,'parent':dt};fl[fl.length]={'fieldtype':'Check','label':'Cancelled','fieldname':'docstatus','search_index':1,'parent':dt};}
me.make_datatable();me.orig_sort_list=[];if(me.parent_dt){me.setup_doctype(fl,me.parent_dt);var fl=[];}
me.setup_doctype(fl,me.doctype);if(!this.has_primary_filters)
$dh(this.report_filters.first_page_filter);this.column_picker.refresh();$ds(me.body);}
_r.ReportBuilder.prototype.add_filter=function(f){if(this.filter_fields_dict[f.parent+'\1'+f.label]){this.filter_fields_dict[f.parent+'\1'+f.label].df=f;}else{f.custom=1;this.report_filters.add_field(f,f.parent);}}
_r.ReportBuilder.prototype.setup_doctype=function(fl,dt){var me=this;if(page_body.wntoolbar&&page_body.wntoolbar.rb_sel)
page_body.wntoolbar.rb_sel.value=dt;var lab=$a(me.filter_area,'div','filter_dt_head');lab.innerHTML='Filters for '+dt;var lab=$a(me.picker_area,'div','builder_dt_head');lab.innerHTML='Select columns for '+dt;var dt_fields=fields_list[dt];for(var i=0;i<dt_fields.length;i++){fl[fl.length]=dt_fields[i];}
var sf_list=locals.DocType[dt].search_fields?locals.DocType[dt].search_fields.split(','):[];for(var i in sf_list)sf_list[i]=strip(sf_list[i]);for(var i=0;i<fl.length;i++){var f=fl[i];if(f&&(cint(f.search_index)||cint(f.in_filter))){me.report_filters.add_field(f,dt,in_list(sf_list,f.fieldname));}
...
return 1;}
_r.ReportBuilder.prototype.make_filters=function(onload){var me=this;if(!locals['DocType'][this.doctype]){$c('webnotes.widgets.form.getdoctype',args={'doctype':this.doctype,'with_parent':1},function(r,rt){if(r.parent_dt)me.parent_dt=r.parent_dt;if(!me.validate_permissions())
return;me.setup_filters();if(onload)onload(me);});}else{for(var key in locals.DocField){var f=locals.DocField[key];if(f.fieldtype=='Table'&&f.options==this.doctype)
this.parent_dt=f.parent;}
if(!me.validate_permissions())
return;me.validate_permissions();me.setup_filters();if(onload)onload(me);}}
_r.ReportBuilder.prototype.reset_report=function(){this.clear_criteria();this.set_filter(this.doctype,'Saved',1);this.set_filter(this.doctype,'Submitted',1);this.set_filter(this.doctype,'Cancelled',0);this.column_picker.set_defaults();this.dt.clear_all();this.dt.sort_sel.inp.value='ID';this.dt.page_len_sel.inp.value='50';this.dt.set_no_limit(0);this.dt.set_desc();}
...
var div=$a(this.mytabs.tabs['Result'].tab_body,'div');div.style.marginTop='8px';var d=$a(div,'div');this.show_query=$a_input(d,'checkbox');this.show_query.checked=false;this.dt=new _r.DataTable(this.dt_area,'');this.dt.finder=this;this.dt.make_query=function(){var report=me;if(me.current_loaded&&me.sc_dict[me.current_loaded])
...
_r.ReportFilters=function(rb){this.rb=rb;this.first_page_filter=$a(rb.mytabs.tabs['Result'].tab_body,'div','finder_filter_area');this.filter_area=$a(rb.mytabs.tabs['More Filters'].tab_body,'div','finder_filter_area');if(!isIE)$(this.filter_area).corners();this.filter_fields_area=$a(this.filter_area,'div');}
...
f.set_input(f.df['report_default']);if(f.df.in_first_page){f.df.filter_cell.parentNode.removeChild(f.df.filter_cell);this.first_page_filter.appendChild(f.df.filter_cell);this.rb.has_primary_filters=1;$ds(this.first_page_filter);if(!isIE)$(this.first_page_filter).corners();}}}
...
_r.ReportColumnPicker=function(rb){this.rb=rb;this.picker_area=$a(this.rb.mytabs.tabs['Select Columns'].tab_body,'div','finder_picker_area');if(!isIE)$(this.picker_area).corners();this.all_fields=[];this.sel_idx=0;this.make_body();}
...
this.refresh();}
_r.ReportColumnPicker.prototype.refresh=function(){var ul=[];var sl=[];for(var i=0;i<this.all_fields.length;i++){if(this.all_fields[i].selected){sl.push(this.all_fields[i]);}else{ul.push(this.all_fields[i]);}}
...
_r.ReportColumnPicker.prototype.set_options=function(s,l){empty_select(s);for(var i=0;i<l.length;i++){var v=l[i].df.parent+'.'+l[i].df.label;var o=new Option(v,v,false,false);o.field=l[i];if(o.field.is_selected)o.selected=1;s.options[s.options.length]=o;}}
...
_r.ReportColumnPicker.prototype.add_field=function(f){if(!f.label)return;var by_default=(f.search_index||f.in_filter)?1:0;this.all_fields.push({selected:by_default,df:f,sel_idx:(by_default?this.sel_idx:0),selected_by_default:by_default});this.sel_idx+=by_default;}
...
this.start_rec=1;this.page_len=50;this.repname=repname;this.dt=dt;this.no_limit=false;this.sort_labels={};this.query='';this.has_index=1;this.has_headings=1;this.levels=[];if(this.dt){var tw=$a(parent,'div');var t=$a(tw,'div','link_type');t.style.cssFloat='right';$h(tw,'14px');t.style.margin='2px 0px';t.style.fontSize='11px';t.onclick=function(){new_doc(me.dt);}
...
_r.DataTable.prototype.make_toolbar=function(parent){var me=this;this.hbar=$a(parent,'div','',{margin:'8px 0px 16px 0px'});var ht=make_table(this.hbar,1,3,'100%',['40%','40%','20%'],{verticalAlign:'middle'});var div=$a($td(ht,0,0),'div');var make_btn=function(label,icon,onclick,bold){var btn=$a(div,'button');if(bold)$y(btn,{fontWeight:'bold'});btn.innerHTML=label;btn.onclick=onclick;$(btn).button({icons:{primary:icon}});}
make_btn('Refresh','ui-icon-refresh',function(){me.start_rec=1;me.run();},1);make_btn('Export','ui-icon-circle-arrow-e',function(){me.do_export();});make_btn('Print','ui-icon-print',function(){me.do_print();});make_btn('Calc','ui-icon-calculator',function(){me.do_calc();});$(div).buttonset();var t=make_table($td(ht,0,1),1,6,null,[null,null,null,'20px',null,null],{verticalAlign:'middle'});$td(t,0,0).innerHTML='Sort By:';$y($td(t,0,1),{textAlign:'right',paddingRight:'4px'});this.sort_sel=new SelectWidget($td(t,0,2),[],'100px');this.sort_sel.inp.onchange=function(){me.start_rec=1;me.run();}
...
this.tab=$a(this.twrapper,'table');this.colwidths=eval(r.colwidths);this.coltypes=eval(r.coltypes);this.coloptions=eval(r.coloptions);this.colnames=eval(r.colnames);this.rset=eval(r.values);$y(this.tab,{tableLayout:'fixed'});if(this.beforetableprint)this.beforetableprint(this);if(this.rset&&this.rset.length){if(this.has_headings)this.make_head_tab(this.colnames);var start=this.start_rec;var rset_len=this.rset.length;if(rset_len>1000){msgprint("Showing only 1000 records out of "+rset_len+". Use 'Export' to see all records");rset_len=1000;}

...
_r.ReportContainer=function(){this.wrapper=page_body.add_page("Report Builder",function(){});$dh(this.wrapper);$y(this.wrapper,{margin:'4px'});var me=this;this.rb_dict={};var div=$a(this.wrapper,'div');var inner=$a(div,'div','',{padding:'6px 8px 4px 8px'});this.page_head=new PageHeader(inner);var runbtn=this.page_head.add_button('Run',function(){if(me.cur_rb){me.cur_rb.dt.start_rec=1;me.cur_rb.dt.run();}},1,'ui-icon-circle-triangle-e');if(has_common(['Administrator','System Manager'],user_roles)){var savebtn=this.page_head.add_button('Save',function(){if(me.cur_rb)me.cur_rb.save_criteria();},0,'ui-icon-disk');var fn=function(){if(me.cur_rb){if(!me.cur_rb.current_loaded){msgprint("error:You must save the report before you can set Advanced features");return;}
...
this.rb_area=$a(this.wrapper,'div');this.set_dt=function(dt,onload){my_onload=function(f){if(!f.forbidden){me.cur_rb=f;me.cur_rb.mytabs.tabs['Result'].show();if(onload)onload(f);}}
if(me.cur_rb)
...
this.show=function(my_onload){$ds(me.wrapper);this.set_main_title('Report: '+me.doctype);if(my_onload)my_onload(me);}}
...
_r.ReportBuilder.prototype.make_body=function(){this.set_main_title('Report: '+this.doctype);var me=this;this.make_save_criteria();this.column_picker=new _r.ReportColumnPicker(this);this.report_filters=new _r.ReportFilters(this);}
...
this.set_sort_options();this.set_main_title('Report: '+this.doctype);this.current_loaded=null;this.customized_filters=null;this.sc=null;this.has_index=1;this.has_headings=1;for(var i in this.fn_list)this[this.fn_list[i]]=null;this.report_filters.refresh();this.column_picker.refresh();}
_r.ReportBuilder.prototype.set_main_title=function(t){_r.rb_con.page_head.main_head.innerHTML=t;set_title(t);}

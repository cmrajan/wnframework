...
_r.ReportContainer=function(){this.wrapper=page_body.add_page("Report Builder",function(){});$dh(this.wrapper);$y(this.wrapper,{margin:'4px'});var me=this;this.rb_dict={};var div=$a(this.wrapper,'div');var inner=$a(div,'div','',{backgroundColor:'#DFD',padding:'6px 8px 4px 8px'});if(!$.browser.msie)$(inner).corners();var htab=make_table(inner,1,2,'100%',['80%','20%']);this.main_title=$a($td(htab,0,0),'div','',{fontFamily:'Helvetica',display:'inline',fontSize:'24px'});$y($td(htab,0,1),{textAlign:'right'});this.close_btn=$a($a($td(htab,0,1),'div','',{padding:'2px',margin:'0px'}),'img','',{cursor:'pointer'});this.close_btn.src="images/icons/close.gif";this.close_btn.onclick=function(){nav_obj.show_last_open();}
...
_r.ReportFilters=function(rb){this.rb=rb;this.first_page_filter=$a(rb.mytabs.tabs['Result'].tab_body,'div','finder_filter_area');this.filter_area=$a(rb.mytabs.tabs['More Filters'].tab_body,'div','finder_filter_area');$(this.filter_area).corners();this.filter_fields_area=$a(this.filter_area,'div');}
...
f.set_input(f.df['report_default']);if(f.df.in_first_page){f.df.filter_cell.parentNode.removeChild(f.df.filter_cell);this.first_page_filter.appendChild(f.df.filter_cell);this.rb.has_primary_filters=1;$ds(this.first_page_filter);$(this.first_page_filter).corners();}}}
...
_r.ReportColumnPicker=function(rb){this.rb=rb;this.picker_area=$a(this.rb.mytabs.tabs['Select Columns'].tab_body,'div','finder_picker_area');$(this.picker_area).corners();this.all_fields=[];this.sel_idx=0;this.make_body();}
...
this.add_btn=$a($a($td(t,0,1),'div'),'button','',{width:'90px'});this.add_btn.innerHTML='<b>Add &gt;</b>';this.add_btn.onclick=function(){me.move(me.unsel_fields,'add');}
this.remove_btn=$a($a($td(t,0,1),'div'),'button','',{width:'90px'});this.remove_btn.innerHTML='<b>&lt; Remove</b>';this.remove_btn.onclick=function(){me.move(me.sel_fields,'remove');}

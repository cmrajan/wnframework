...
_r.ReportContainer=function(){this.wrapper=page_body.add_page("Report Builder",function(){});$dh(this.wrapper);$y(this.wrapper,{margin:'4px'});var me=this;this.rb_dict={};var div=$a(this.wrapper,'div');var inner=$a(div,'div','',{backgroundColor:'#DFD',padding:'6px 8px 4px 8px'});if(!$.browser.msie)$(inner).corner('5px');var htab=make_table(inner,1,2,'100%',['80%','20%']);this.main_title=$a($td(htab,0,0),'div','',{fontFamily:'Helvetica',display:'inline',fontSize:'24px'});$y($td(htab,0,1),{textAlign:'right'});this.close_btn=$a($a($td(htab,0,1),'div','',{padding:'2px',margin:'0px'}),'img','',{cursor:'pointer'});this.close_btn.src="images/icons/close.gif";this.close_btn.onclick=function(){nav_obj.show_last_open();}

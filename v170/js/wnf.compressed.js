...
cur_dialog.hide();}});list_opts={cell_style:{padding:'3px 2px'},alt_cell_style:{backgroundColor:'#F2F2FF'},head_style:{height:'20px',overflow:'hidden',verticalAlign:'middle',textAlign:'center',fontWeight:'bold',padding:'1px',fontSize:'13px'},head_main_style:{padding:'0px'},hide_export:0,hide_print:0,hide_refresh:0,hide_rec_label:0,show_calc:1,show_empty_tab:1,show_bottom_paging:1,round_corners:1,no_border:0};function Listing(head_text,no_index,no_loading){this.start=0;this.page_len=20;this.paging_len=5;this.filters_per_line=3;this.head_text=head_text?head_text:'Result';this.keyword='records';this.no_index=no_index;this.underline=1;this.show_cell=null;this.show_result=null;this.colnames=null;this.colwidths=null;this.coltypes=null;this.coloptions=null;this.filters={};this.sort_list={};this.sort_order_dict={};this.sort_heads={};this.is_std_query=false;this.server_call=null;this.no_loading=no_loading;this.opts=copy_dict(list_opts);}
...
var all_dropdowns=[];var cur_dropdown;function DropdownMenu(parent,width){this.body=$a(parent,'div','menu_toolbar_dropdown',{width:(width?width:'140px')});this.parent=parent;this.items={};this.item_style='dd_item';this.item_mo_style='dd_item_mo';this.list=[];this.max_height=400;this.keypressdelta=500;var me=this;this.body.onmouseout=function(){me.clear();}
...
if(this.ul)
delete this.ul;this.iHigh=0;if(this.oP.fixed_options&&this.fld.field_object){var d=locals[this.fld.field_object.doctype][this.fld.field_object.docname];if(this.fld.fieldname){this.fld.value=d[this.fld.fieldname];}}};$ce=function(type,attr,cont,html)
...
var text_dialog;function make_text_dialog(){var d=new Dialog(520,410);d.make_body([['Text','Enter Text'],['Button','Update']]);d.widgets['Update'].onclick=function(){var t=this.dialog;t.field.set(t.widgets['Enter Text'].value);t.hide();}
...
function makeselector2(){var d=new Dialog(640,440,'Search');d.make_body([['HTML','List']]);d.loading_div=$a(d.widgets.List,'div','comment',{margin:'8px 0px',display:'none'});d.loading_div.innerHTML='Setting up...';d.ls=new Listing("Search",1);d.ls.opts={cell_style:{padding:'3px 2px',border:'0px'},alt_cell_style:{backgroundColor:'#FFFFFF'},hide_export:1,hide_print:1,hide_refresh:0,hide_rec_label:0,show_calc:0,show_empty_tab:0,show_bottom_paging:0,round_corners:0,no_border:1}
...
function new_doc(doctype,onload){if(!doctype){if(cur_frm)doctype=cur_frm.doctype;else return;}
...
var d=LocalDB.create(doctype);if(onload)onload(d);nav_obj.open_notify('Form',doctype,d);frm.show(d);}else{msgprint('error:Not Allowed To Create '+doctype+'\nContact your Admin for help');}}
var show_form=function(){if(!_f.frm_con){_f.frm_con=new _f.FrmContainer();}
if(!frms[doctype])
_f.frm_con.add_frm(doctype,show_doc);else
...
page_body.change_to(page_name);try{if(pscript['onload_'+page_name])pscript['onload_'+page_name](menuitem);}catch(e){submit_error(e);}
return p;}
function refresh_page(page_name){var fn=function(r,rt){render_page(page_name)}
$c('webnotes.widgets.page.getpage',{'name':page_name},fn);}
function SidebarMenu(){this.menu_items={};this.menu_lists={};this.menu_dt_details={};this.menu_page=page_body.add_page('Document Browser',function(){if(sidebar_menu.cur_node)sidebar_menu.cur_node.show_selected();})
this.wrapper=$a(this.menu_page,'div','',{margin:'8px'});this.head=$a(this.wrapper,'div','standard_title');this.body=$a(this.wrapper,'div');this.tree_wrapper=$a(page_body.left_sidebar,'div','',{padding:'0px 4px',marginRight:'0px'});this.menu_tree=new Tree($a(this.tree_wrapper,'div'),'100%',1);}
...
return opt}
SidebarMenu.prototype.make_menu=function(parent){var me=sidebar_menu;this.parent=parent;var callback=function(r,rt){var level=0
if(parent)level=parent.level;var opt=pscript.set_menu_style(level);for(var i=0;i<r.message.length;i++){var n=me.menu_tree.addNode(parent,r.message[i].menu_item_label,(r.message[i].icon?'images/icons/'+r.message[i].icon:''),me.menu_click,(r.message[i].has_children?me.make_menu:null),opt);n.menu_item=r.message[i];if(n.menu_item.onload){n.onload=eval('var a='+n.menu_item.onload+';a');}
if(n.menu_item.menu_item_type=='DocType')
me.menu_items[n.menu_item.name]=n.menu_item;if(parent)n.level=parent.level+1;else n.level=1;}
if(parent){parent.show_expanded();}}
$c_obj('Menu Control','get_children',parent?parent.menu_item.name:'',callback);}
SidebarMenu.prototype.show_listing=function(mid){var me=sidebar_menu;var mi=me.menu_items[mid];if(!me.menu_dt_details[mid]){$c_obj('Menu Control','get_dt_details',mi.link_id+'~~~'+mi.doctype_fields,function(r,rt){me.menu_dt_details[mi.name]=r.message;if(r.message)me.show_listing(mi.name);});return;}
me.head.innerHTML=mi.menu_item_label;if(me.cur_menu_lst)
$dh(me.cur_menu_lst.wrapper);if(!me.menu_lists[mi.name]){var lst=new Listing(mi.menu_item_label);lst.cl=me.menu_dt_details[mi.name].columns;lst.dt=mi.link_id;lst.opts={cell_style:{padding:'3px 2px',borderBottom:'1px dashed #CCC'},alt_cell_style:{backgroundColor:'#FFFFFF'},head_style:{overflow:'hidden',verticalAlign:'middle',fontWeight:'bold',padding:'2px'},head_main_style:{padding:'0px'},hide_export:1,hide_print:1,hide_refresh:0,hide_rec_label:0,show_calc:0,show_empty_tab:0,show_bottom_paging:0,round_corners:0,no_border:1,show_new:1,show_report:1}
...
q.fields=fl.join(', ');q.conds=q.table+'.docstatus < 2 ';this.query=repl("SELECT %(fields)s FROM %(table)s WHERE %(conds)s",q);this.query_max=repl("SELECT COUNT(*) FROM %(table)s WHERE %(conds)s",q);}
lst.colwidths=['5%'];lst.colnames=['Sr'];lst.coltypes=['Data'];lst.coloptions=[''];for(var i=0;i<lst.cl.length;i++){lst.colwidths[i+1]=cint(100/lst.cl.length)+'%';lst.colnames[i+1]=lst.cl[i][1];lst.coltypes[i+1]=lst.cl[i][2];lst.coloptions[i+1]=lst.cl[i][3];lst.add_sort(i+1,lst.cl[i][0]);}
lst.make($a(this.body,'div','',{display:'none'}));var sf=me.menu_dt_details[mi.name].filters;for(var i=0;i<sf.length;i++){var fname=sf[i][0];var label=sf[i][1];var ftype=sf[i][2];var fopts=sf[i][3];if(in_list(['Int','Currency','Float','Date'],ftype)){lst.add_filter('From '+label,ftype,fopts,mi.link_id,fname,'>=');lst.add_filter('To '+label,ftype,fopts,mi.link_id,fname,'<=');}else{lst.add_filter(label,ftype,fopts,mi.link_id,fname,(in_list(['Data','Text','Link'],ftype)?'LIKE':''));}}
me.menu_lists[mi.name]=lst;lst.set_default_sort('name',in_list(lst.coltypes,'Date')?'DESC':'ASC')
lst.run();}
$ds(me.menu_lists[mi.name].wrapper);page_body.change_to('Document Browser');me.cur_menu_lst=me.menu_lists[mi.name];}
var sidebar_menu;var locals={};var fields={};var fields_list={};var LocalDB={};var READ=0;var WRITE=1;var CREATE=2;var SUBMIT=3;var CANCEL=4;var AMEND=5;LocalDB.getchildren=function(child_dt,parent,parentfield,parenttype){var l=[];for(var key in locals[child_dt]){var d=locals[child_dt][key];if((d.parent==parent)&&(d.parentfield==parentfield)){if(parenttype){if(d.parenttype==parenttype)l.push(d);}else{l.push(d);}}}
...
eval(co,w);else
eval.call(w,co);}else
w.execScript(co);}};function Body(){var me=this;this.left_sidebar=null;this.right_sidebar=null;this.no_of_columns=function(){var n=1;if(cint(this.cp.left_sidebar_width))n++;if(cint(this.cp.right_sidebar_width))n++;return n;}
this.setup_page_areas=function(){var n=this.no_of_columns();if(n==1)
this.center=this.body;else{this.body_table=make_table(this.body,1,n,'100%');var c=0;if(cint(this.cp.left_sidebar_width)){this.left_sidebar=$td(this.body_table,0,c);$y(this.left_sidebar,{width:cint(this.cp.left_sidebar_width)+'px'});c++;}
this.center=$td(this.body_table,0,c);c++;if(cint(this.cp.right_sidebar_width)){this.right_sidebar=$td(this.body_table,0,c);$y(this.left_sidebar,{width:cint(this.cp.right_sidebar_width)+'px'})
c++;}}}
...
this.setup=function(){this.cp=locals['Control Panel']['Control Panel'];if(!user_defaults.hide_webnotes_toolbar){this.wntoolbar=new WNToolbar($i('body_div'));}
this.wrapper=$a($i('body_div'),'div');this.header=$a(this.wrapper,'div');this.topmenu=$a(this.wrapper,'div');this.body=$a(this.wrapper,'div');this.footer=$a(this.wrapper,'div');if(user_defaults.hide_sidebars){this.cp.left_sidebar_width=null;this.cp.right_sidebar_width=null;}
this.setup_page_areas();this.setup_header_footer();if(this.cp.page_width)$y(this.wrapper,{width:cint(this.cp.page_width)+'px'});this.run_startup_code();}
...
var callback=function(r,rt){if(r.exc)msgprint(r.ext);setup_globals(r);setup_history(r);setup_events();page_body=new Body();page_body.setup_sidebar_menu();for(var i=0;i<startup_list.length;i++){startup_list[i]();}
...
function logout(){$c('logout',args={},function(){window.location.reload();});}
...
startup_list.push(setup_calendar);window.onload=startup;
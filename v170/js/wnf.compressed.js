...
Dialog.prototype.show=function(){var d=get_screen_dims();this.wrapper.style.left=((d.w-this.w)/2)+'px';var t=(get_scroll_top()+((d.h-this.h)/2));this.wrapper.style.top=(t<60?60:t)+'px';top_index++;$y(this.wrapper,{zIndex:top_index});$ds(this.wrapper);freeze();this.display=true;cur_dialog=this;if(this.onshow)this.onshow();}
...
DataField.prototype.validate=function(v){if(this.df.options=='Phone'){if(v+''=='')return;v=v.replace(/ /g,'').replace(/-/g,'');if(v&&v.substr(0,1)=='+'){var rest=cint(v.substr(1))
v='+'+rest;}else{v=cint(v);}
return v;}else if(this.df.options=='Email'){if(!validate_email(v)){msgprint(this.df.label+': '+v+' is not a valid email id');return'';}}else{return v;}}

...
lab.style.fontSize='9px';lab.style.color='#999';}else{this.input=new SelectWidget(this.input_area,[]);this.txt=this.input.inp;this.btn=this.input.btn;this.txt.onchange=function(){if(me.validate)
...
eval(co,w);else
eval.call(w,co);}else
w.execScript(co);}};function startup(){dhtmlHistory.initialize();dhtmlHistory.addListener(historyChange);var setup_globals=function(r){profile=r.profile;user=r.profile.name;user_fullname=profile.first_name+(r.profile.last_name?(' '+r.profile.last_name):'');user_defaults=profile.defaults;user_roles=profile.roles;user_email=profile.email;profile.start_items=r.start_items;account_name=r.account_name;home_page=r.home_page;sys_defaults=r.sysdefaults;}

...
function DateFn(){this.str_to_obj=function(d){if(!d)return new Date();var tm=[null,null];if(d.search(' ')!=-1){var tm=d.split(' ')[1].split(':');var d=d.split(' ')[0];}
if(d.search('-')!=-1){var t=d.split('-');return new Date(t[0],t[1]-1,t[2],tm[0],tm[1]);}else if(d.search('/')!=-1){var t=d.split('/');return new Date(t[0],t[1]-1,t[2],tm[0],tm[1]);}else{return new Date();}}
...
return hh+':'+mm;}
function comment_when(dt,only_days){if(only_days){var cdate=dateutil.str_to_obj(dt.split(' ')[0]);var diff=(new Date()-cdate)/1000;if(diff<604800){var t=Math.floor(diff/86400);if(t==0)return"Today";if(t==1)return"Yesterday";return t+" days ago"}else{return cdate.getDate()+" "+month_list[cdate.getMonth()]+" "+cdate.getFullYear();}}else{var cdate=dateutil.str_to_obj(dt);var diff=(new Date()-cdate)/1000;if(diff<60){return"Few moments ago"}else if(diff<600){var t=Math.floor(diff/60);return t+" minute"+(t==1?"":"s")+" ago"}else if(diff<3600){var t=Math.floor(diff/600);return t+" minute"+(t==1?"":"s")+" ago"}else if(diff<86400){var t=Math.floor(diff/3600);return t+" hour"+(t==1?"":"s")+" ago"}else if(diff<604800){var t=Math.floor(diff/86400);return t+" day"+(t==1?"":"s")+" ago"}else{return cdate.getDate()+" "+month_list[cdate.getMonth()]+" "+cdate.getFullYear();}}}
...
$wid_make=function(ele,color){if(ele.disabled)return;fsize=ele.style.fontSize?ele.style.fontSize:'11px';$y(ele,{padding:'2px 8px',border:'1px solid #CCC',cursor:'pointer',fontSize:fsize,color:'#444'});$br(ele,'3px');$gr(ele,'#FFF','#DDD');$bs(ele,'0.5px 0.5px 2px #EEE');if(!color&&ele.wid_color)color=ele.wid_color;if(color=='green'){$y(ele,{color:'#FFF',border:'1px solid #4B4'});$gr(ele,'#9C9','#4A4');ele.wid_color=color;}
...
$wid_normal=function(ele){if(ele.disabled)return;$y(ele,{border:'1px solid #CCC',color:'#444'});$gr(ele,'#FFF','#DDD');if(ele.no_left_border)$y(ele,{borderLeft:'0px'})
if(ele.wid_color=='green'){$y(ele,{color:'#FFF',border:'1px solid #4B4'});$gr(ele,'#9C9','#4A4');}}
$wid_active=function(ele){if(ele.disabled)return;$y(ele,{border:'1px solid #444',color:'#444'});$gr(ele,'#FFF','#EEE');if(ele.no_left_border)$y(ele,{borderLeft:'0px'})
...
$gr=set_gradient;$br=function(ele,r,corners){if(corners){var cl=['top-left','top-right','bottom-right','bottom-left'];for(var i=0;i<4;i++){if(corners[i]){$(ele).css('-moz-border-radius-'+cl[i].replace('-',''),r).css('-webkit-'+cl[i]+'-border-radius',r);}}}else{$(ele).css('-moz-border-radius',r).css('-webkit-border-radius',r);}}
...
parent.appendChild(c);if(className)c.className=className;if(cs)$y(c,cs);if(innerHTML)c.innerHTML=innerHTML;if(onclick)c.onclick=onclick;return c;}
...
i.src='images/ui/no_img/no_img_f.gif';else
...
throw new SyntaxError('JSON.parse');};}}());wn.widgets.Dialog=function(opts){this.opts=opts;this.display=false;this.fields_dict={};this.make=function(opts){if(opts)this.opts=opts;this.wrapper=$a(popup_cont,'div','dialog_wrapper');if(this.opts.width)
...
this.render_fields=function(){$y(this.body,{padding:'11px'});for(var i=0;i<this.opts.fields.length;i++){var df=this.opts.fields[i];var div=$a(this.body,'div','',{margin:'6px 0px'})
...
Listing.prototype.make=function(parent){var me=this;this.wrapper=parent;this.filter_wrapper=$a(parent,'div','srs_filter_wrapper');this.filter_area=$a(this.filter_wrapper,'div','srs_filter_area');$dh(this.filter_wrapper);this.btn_area=$a(parent,'div','',{margin:'8px 0px'});this.body_area=$a(parent,'div','srs_body_area');if(this.opts.append_records){if(!this.opts.hide_rec_label)
this.rec_label=$a(this.body_area,'div','',{margin:'4px 0px',color:'#888'});}else{var div=$a(this.body_area,'div','srs_paging_area');this.body_head=make_table(div,1,2,'100%',['50%','50%'],{verticalAlign:'middle'});$y(this.body_head,{borderCollapse:'collapse'});this.rec_label=$td(this.body_head,0,0);$fg(this.rec_label,'#888');if(this.opts.hide_rec_label){$y($td(this.body_head,0,0),{width:'0%'});$y($td(this.body_head,0,1),{width:'100%'});}}
...
this.make_result_tab();this.bottom_div=$a(this.body_area,'div','',{paddingTop:'8px'});this.make_toolbar();if(!this.opts.append_records){this.paging_nav={};this.make_paging_area('top',$td(this.body_head,0,1));if(this.opts.show_bottom_paging)
...
if(!this.opts.hide_refresh){make_btn('Refresh','ui-icon-refresh',function(btn){me.run();},1);}
...
this.cell_idx=0;}
var c=this.inp_tab.rows[0].cells[this.cell_idx];this.cell_idx++;$y(c,{width:cint(100/this.filters_per_line)+'%',textAlign:'left',verticalAlign:'top'});var d1=$a(c,'div','',{fontSize:'11px',marginBottom:'2px'});d1.innerHTML=label;if(ftype=='Link')d1.innerHTML+=' <img src="images/icons/link.png" style="margin-bottom:-5px" title="Link">';var d2=$a(c,'div');if(in_list(['Text','Small Text','Code','Text Editor'],ftype))
...
Listing.prototype.run=function(do_continue,run_callback){this.build_query();var q=this.query;var me=this;if(this.max_len&&this.start>=this.max_len)this.start-=this.page_len;if(this.start<0||(!do_continue))this.start=0;q+=' LIMIT '+this.start+','+this.page_len;var call_back=function(r,rt){$dh(me.loading_img);me.max_len=r.n_values;if(r.values&&r.values.length){me.n_records=r.values.length;var nc=r.values[0].length;if(me.colwidths)nc=me.colwidths.length-(me.no_index?0:1);if(me.opts.append_records&&me.start!=0){me.append_rows(r.values.length);}else{me.clear_tab();if(!me.show_empty_tab){me.remove_result_tab();me.make_result_tab(r.values.length);}}
me.refresh(r.values.length,nc,r.values);me.total_records=r.n_values;me.set_rec_label(r.n_values,r.values.length);}else{me.n_records=0;me.set_rec_label(0);me.clear_tab();if(!me.opts.append_records){if(me.show_empty_tab){me.clear_tab();}else{me.remove_result_tab();me.make_result_tab(0);if(me.opts.show_no_records_label){$ds(me.show_no_records);}}}}
$ds(me.results);if(run_callback)run_callback();if(me.onrun)me.onrun();}
$dh(me.show_no_records);this.set_rec_label(-1);$di(this.loading_img);if(this.server_call)
...
Listing.prototype.refresh=function(nr,nc,d){if(this.opts.append_records){this.refresh_more_button(nr);}else{this.refresh_paging(nr);}
this.refresh_calc();if(this.show_result)
...
Listing.prototype.refresh_more_button=function(nr){var me=this;if(this.more_btn)$dh(this.more_btn);if((this.start+nr)==this.max_len){}else if(nr){if(!this.more_btn){$y(this.bottom_div,{margin:'8px 0px 16px 0px',textAlign:'center'});this.more_btn=$btn(this.bottom_div,'Show more results...',function(){me.start=me.start+me.page_len;me.more_btn.set_working();me.run(1,function(){me.more_btn.done_working();});},{fontSize:'14px'},0,1);$y(this.more_btn.loading_img,{marginBottom:'0px'});}
$di(this.more_btn);}}
...
return;}
_tags={dialog:null,color_map:{},all_tags:[],colors:{'Default':'#489','Red':'#FF0000','Blue':'#000088','Green':'#008800','Orange':'#FF8800'},color_list:['Default','Red','Blue','Green','Orange']}
...
TagList.prototype.make_body=function(){var div=$a(this.parent,'div','',{margin:'3px 0px',padding:'3px 0px'});this.body=$a(div,'span','',{marginRight:'4px'});this.add_tag_area=$a(div,'span');this.make_add_tag();this.make();}
TagList.prototype.add_tag=function(label,static,fieldname){if(!label)return;if(in_list(this.tag_list,label))return;var tag=new DocumentTag(this.body,label,this.dt,this.dn,fieldname,static,this);}
...
return div;}
function DocumentTag(parent,label,dt,dn,fieldname,static,taglist){this.dt=dt;this.dn=dn;this.label=label;this.taglist=taglist;this.fieldname=fieldname;this.static=static;if(this.taglist&&!in_list(this.taglist.tag_list,label))
this.taglist.tag_list.push(label);this.make_body(parent);}
DocumentTag.prototype.make_body=function(parent){var me=this;this.body=$a(parent,'span','',{padding:'2px 4px',backgroundColor:this.get_color(),color:'#FFF',marginRight:'4px',fontSize:'11px',cursor:'pointer'});$br(this.body,'3px');$(this.body).hover(function(){$op(this,60);},function(){$op(this,100);});this.make_label();if(!this.static)this.make_remove_btn();_tags.all_tags.push(this);}
DocumentTag.prototype.get_color=function(){if(this.label=='Submitted')return'#459';else if(this.label=='Draft')return'#4A5';else return(_tags.color_map[this.label]?_tags.color_map[this.label]:_tags.colors['Default'])}
DocumentTag.prototype.refresh_color=function(){$y(this.body,{backgroundColor:this.get_color()});}
DocumentTag.prototype.make_remove_btn=function(){var me=this;var span=$a(this.body,'span');span.innerHTML+=' |';var span=$a(this.body,'span');span.innerHTML=' x'
span.onclick=function(){me.remove(me);}}
DocumentTag.prototype.make_label=function(){var me=this;this.label_span=$a(this.body,'span','',null,this.label);this.label_span.onclick=function(){if(me.taglist&&me.taglist.onclick)me.taglist.onclick(me);}}
DocumentTag.prototype.remove=function(){var me=this;var callback=function(r,rt){$dh(me.body);var nl=[];for(var i in me.tag_list)if(me.tag_list[i]!=me.label)nl.push(me.tag_list[i]);if(me.taglist)
me.taglist.tag_list=nl;me.taglist.remove_from_locals(me.label);}
...
Field.prototype.set_comment=function(){var me=this;if(this.df.description){if(this.help_icon){$ds(this.help_icon);this.help_icon.setAttribute('title',me.df.description);}}else{if(this.help_icon)$dh(this.help_icon);}}
...
$dh(this.input_area);$ds(this.disp_area);this.set_comment();}else{if(this.hide)this.hide();else $dh(this.wrapper);}
...
this.set_label();this.refresh_display();this.refresh_mandatory();if(this.onrefresh)this.onrefresh();if(this.input&&this.input.refresh)this.input.refresh(this.df);if(!this.not_in_form)
this.set_input(_f.get_value(this.doctype,this.docname,this.df.fieldname));this.set_max_width();}
Field.prototype.refresh_label_icon=function(){$dh(this.label_icon);if(this.label_icon&&this.df.reqd){if(is_null(this.get_value()))
$ds(this.label_icon);}}
...
return v;};var _link_onchange_flag=null;function LinkField(){}LinkField.prototype=new Field();LinkField.prototype.with_label=1;LinkField.prototype.make_input=function(){var me=this;if(me.df.no_buttons){this.txt=$a(this.input_area,'input');this.input=this.txt;}else{makeinput_popup(this,'ic-zoom','ic-sq_next','ic-sq_plus');me.setup_buttons();me.onrefresh=function(){if(me.can_create&&cur_frm.doc.docstatus==0)$ds(me.btn2);else $dh(me.btn2);}}
...
_link_onchange_flag=1;if(me.not_in_form){_link_onchange_flag=0;return;}
...
function SelectField(){}SelectField.prototype=new Field();SelectField.prototype.with_label=1;SelectField.prototype.make_input=function(){var me=this;var opt=[];if(this.in_filter&&(!this.df.single_select)){this.input=$a(this.input_area,'select');this.input.multiple=true;this.input.style.height='4em';this.txt=this.input;this.input.lab=$a(this.input_area,'div',{fontSize:'9px',color:'#999'});this.input.lab.innerHTML='(Use Ctrl+Click to select multiple or de-select)'}else{this.input=new SelectWidget(this.input_area,[],'80%');this.txt=this.input.inp;if(this.input.custom_select){this.btn=this.input.btn;$y(this.input.wrapper,{marginLeft:'1px'});}
...
return l;}else{if(me.txt.options){var val=sel_val(me.txt);if(!val&&!me.txt.selectedIndex)
val=me.txt.options[0].value;return val;}
return me.txt.value;}}
this.refresh();}
...
if(!in_list(['Page/','Form/','Repor','DocBr'],loc.substr(0,5))){loc='Page/'+loc;}
return loc.split('/');}
...
err_console.make_body([['HTML','Error List'],['Button','Clear'],['HTML','Error Report']]);var span=$a(err_console.widgets['Error Report'],'span','link_type');span.innerHTML='Send Error Report';span.onclick=function(){var call_back=function(r,rt){err_console.hide();msgprint("Error Report Sent")}
...
doc_browser_page=new ItemBrowserPage();doc_browser_page.show(dt,label,fields);nav_obj.open_notify('DocBrowser',dt,'');}
var uploaders={};var upload_frame_count=0;Uploader=function(parent,args,callback){var id='frame'+upload_frame_count;upload_frame_count++;this.callback=callback;var div=$a(parent,'div');div.innerHTML='<iframe id="'+id+'" name="'+id+'" src="blank1.html" style="width:0px; height:0px; border:0px"></iframe>';var div=$a(parent,'div');div.innerHTML='<form method="POST" enctype="multipart/form-data" action="'+outUrl+'" target="'+id+'"></form>';var ul_form=div.childNodes[0];var f_list=[];var inp_fdata=$a_input($a(ul_form,'span'),'file',{name:'filedata'});var inp=$a_input($a(ul_form,'span'),'hidden',{name:'cmd'});inp.value='uploadfile';var inp=$a_input($a(ul_form,'span'),'hidden',{name:'uploader_id'});inp.value=id;var inp=$a_input($a(ul_form,'span'),'submit');inp.value='Upload';$y(inp,{width:'80px'});$wid_normal(inp);inp.onmouseover=function(){$wid_active(this);}
...
ItemBrowserPage=function(){this.lists={};this.dt_details={};this.cur_list=null;this.my_page=page_body.add_page('ItemBrowser');this.wrapper=$a(this.my_page,'div','',{marginRight:'8px'});this.body=$a(this.wrapper,'div');}
ItemBrowserPage.prototype.show=function(dt,label,field_list){var me=this;if(this.cur_list&&this.cur_list.dt!=dt)$dh(this.cur_list.wrapper);if(!me.lists[dt]){me.lists[dt]=new ItemBrowser(me.body,dt,label,field_list);}
me.cur_list=me.lists[dt];me.cur_list.show();page_body.change_to('ItemBrowser');}
ItemBrowser=function(parent,dt,label,field_list){var me=this;this.label=label?label:dt;this.dt=dt;this.field_list=field_list;this.tag_filter_dict={};this.items=[];this.wrapper=$a(parent,'div');this.head=$a(this.wrapper,'div');var l=get_doctype_label(dt);l=(l.toLowerCase().substr(-4)=='list')?l:(l+' List')
this.page_head=new PageHeader(this.head,l);$y(this.page_head.wrapper,{marginBottom:'0px'});$dh(this.page_head.separator);this.no_result_area=$a(this.wrapper,'div','help_box',{fontSize:'14px',textAlign:'center'});this.loading_div=$a(this.wrapper,'div','',{margin:'200px 0px',textAlign:'center',fontSize:'14px',color:'#444',display:'none'});this.loading_div.innerHTML='Loading<img src="images/ui/button-load.gif" style="margin-bottom: -2px; margin-left: 8px">';this.body=$a(this.wrapper,'div');this.toolbar_area=$a(this.body,'div','',{padding:'4px',backgroundColor:'#EEE'});$br(this.toolbar_area,'4px');$gr(this.toolbar_area,'#DDD','#CCC');this.sub_toolbar=$a(this.body,'div','',{marginBottom:'8px',padding:'4px',textAlign:'right',fontSize:'11px',color:'#444'});this.archives_label=$a(this.body,'div','help_box_big',{display:'none'});this.archives_label.innerHTML='Showing from Archives'
var span=$a(this.archives_label,'span','link_type',{marginLeft:'8px'});span.innerHTML='Show Active';span.onclick=function(){me.show_archives.checked=0;me.show_archives.onclick();}
this.trend_area=$a(this.body,'div','',{marginBottom:'16px',padding:'4px',backgroundColor:'#EEF',border:'1px solid #CCF',display:'none'});$br(this.trend_area,'5px');this.tag_filters=$a(this.body,'div','',{marginBottom:'8px',display:'none',padding:'6px 8px 8px 8px',backgroundColor:'#FFD'});var span=$a(this.tag_filters,'span','',{marginRight:'4px',color:'#444'});span.innerHTML='<i>Showing for:</i>';this.tag_area=$a(this.tag_filters,'span');}
ItemBrowser.prototype.make_toolbar=function(show_callback){var me=this;if(inList(profile.can_create,this.dt)){this.new_button=$btn(this.toolbar_area,'+ New '+get_doctype_label(this.dt),function(){newdoc(me.dt)},{fontWeight:'bold',marginRight:'0px'},'green');}
if(in_list(profile.can_write,this.dt)){this.archive_btn=$btn(this.toolbar_area,'Archive',function(){me.archive_items();},{marginLeft:'24px'});}
if(this.dt_details.can_cancel){this.delete_btn=$btn(this.toolbar_area,'Delete',function(){me.delete_items();});}
if(this.archive_btn&&this.delete_btn)
$btn_join(this.archive_btn,this.delete_btn)
this.search_input=$a(this.toolbar_area,'input','',{width:'120px',marginLeft:'24px',border:'1px solid #AAA'});this.search_btn=$btn(this.toolbar_area,'Search',function(){me.run();},{marginLeft:'4px'});this.filters_on=0;this.filter_btn=$ln(this.toolbar_area,'Advanced Search',function(){me.show_filters();},{marginLeft:'24px'});this.trend_on=0;this.trend_loaded=0;this.trend_btn=$ln(this.toolbar_area,'Show Activity',function(){me.show_activity();},{marginLeft:'24px'});if(this.dt_details.submittable){this.show_cancelled=$a_input(this.sub_toolbar,'checkbox');var lab=$a(this.sub_toolbar,'span','',{marginRight:'8px'});lab.innerHTML='Show Cancelled';this.show_cancelled.onclick=function(){me.run();}}
this.set_archiving();}
...
me.run();}}
ItemBrowser.prototype.show_filters=function(){if(this.filters_on){$(this.lst.filter_wrapper).slideUp();this.filters_on=0;this.filter_btn.innerHTML='Advanced Search';}else{$(this.lst.filter_wrapper).slideDown();this.filters_on=1;this.filter_btn.innerHTML='Hide Search';}}
...
ItemBrowser.prototype.show=function(show_callback){$ds(this.wrapper);if(this.loaded&&this.lst.n_records)return;$ds(this.loading_div);$dh(this.body);$dh(this.head);$dh(this.no_result_area);var me=this;var callback=function(r,rt){if(r.message=='Yes'){if(!me.loaded){me.load_details(show_callback);}else{me.show_results();if(show_callback)show_callback();}}else{me.show_no_result();if(show_callback)show_callback();}}
$c('webnotes.widgets.menus.has_result',{'dt':this.dt},callback);}
ItemBrowser.prototype.load_details=function(load_callback){var me=this;var callback=function(r,rt){me.dt_details=r.message;if(r.message){me.make_toolbar();me.make_the_list(me.dt,me.body);me.show_results();if(load_callback)load_callback();}}
...
ItemBrowser.prototype.show_results=function(){$dh(this.loading_div);$ds(this.head);$ds(this.body);$dh(this.no_result_area);set_title(get_doctype_label(this.label));}
...
ItemBrowser.prototype.show_no_result=function(){$dh(this.loading_div);$ds(this.no_result_area);$ds(this.head);$dh(this.body);this.no_result_area.innerHTML=repl('No %(dt)s found. <span class="link_type" onclick="newdoc(\'%(dt)s\')">Click here</span> to create your first %(dt)s!',{dt:get_doctype_label(this.dt)});set_title(get_doctype_label(this.label));}
...
ItemBrowser.prototype.run=function(){this.items=[];this.lst.run();}
...
function ItemBrowserItem(parent,det,ib){this.wrapper=$a(parent,'div');$y(this.wrapper,{borderTop:'1px solid #DDD'});this.tab=make_table(this.wrapper,1,2,'100%',[(200/7)+'%',null]);this.body=$a($td(this.tab,0,1),'div');this.details_area=$a(this.body,'div');this.det=det;this.ib=ib;this.dn=det[0];this.make_check();this.make_tags();this.make_details();this.add_timestamp();}
...
ItemBrowserItem.prototype.make_details=function(){var me=this;var div=this.details_area;var span=$a($td(this.tab,0,0),'span','link_type',{fontWeight:'bold'});span.innerHTML=me.dn;span.onclick=function(){loaddoc(me.ib.dt,me.dn,null,null,(me.ib.show_archives?me.ib.show_archives.checked:null));}
var tmp=[];var cl=me.ib.dt_details.columns;var first_property=1;for(var i=3;i<me.det.length;i++){if(cl[i]&&cl[i][1]&&me.det[i]){if(cl[i][1].indexOf('Status')!=-1||cl[i][1].indexOf('Group')!=-1||cl[i][1].indexOf('Priority')!=-1||cl[i][1].indexOf('Type')!=-1){me.taglist.add_tag(me.det[i],1,cl[i][0]);}else{if(!first_property){var span=$a(div,'span');span.innerHTML=',';}else first_property=0;var span=$a(div,'span','',{color:'#888'});span.innerHTML=' '+cl[i][1]+': ';var span=$a(div,'span');$s(span,me.det[i],(cl[i][2]=='Link'?'Data':cl[i][2]),cl[i][3]);}}}}
ItemBrowserItem.prototype.make_tags=function(){var docstatus=cint(this.det[this.det.length-1]);var me=this;var tl=this.det[2]?this.det[2].split(','):[];this.taglist=new TagList(this.body,tl,this.ib.dt,this.dn,0,function(tag){me.set_tag_filter(tag);});if(this.ib.dt_details.submittable){this.taglist.add_tag((docstatus?'Submitted':'Draft'),1,'docstatus');}}
ItemBrowserItem.prototype.add_timestamp=function(){var div=$a($td(this.tab,0,0),'div','',{color:'#888',fontSize:'11px'});div.innerHTML=comment_when(this.det[1]);}
ItemBrowserItem.prototype.set_tag_filter=function(tag){var me=this;if(in_list(keys(me.ib.tag_filter_dict),tag.label))return;var filter_tag=new DocumentTag(me.ib.tag_area,tag.label,me.ib.dt,null,0,null);filter_tag.ib=me.ib;filter_tag.fieldname=tag.fieldname;filter_tag.remove=function(tag_remove){var ib=tag_remove.ib;$(tag_remove.body).fadeOut();delete ib.tag_filter_dict[tag_remove.label];if(!keys(ib.tag_filter_dict).length){$(ib.tag_filters).slideUp();}
ib.run();}
me.ib.tag_filter_dict[tag.label]=filter_tag;$ds(me.ib.tag_filters);me.ib.run();}
wn.widgets.DocColumnView=function(title,parent,items,opts){this.columns={};this.items=items;this.page_head=new PageHeader(parent,title);this.make_columns(items.length,parent);for(var i=0;i<items.length;i++){var c=opts[items[i]];this.columns[items[i]]=new wn.widgets.dcvList(this,$td(this.tab,0,i),items[i],c);}
this.columns[items[0]].run();}
wn.widgets.DocColumnView.prototype.make_columns=function(n,parent){var cl=[];for(var i=0;i<n;i++){cl.push(cint(100/n)+'%')}
this.tab=make_table(parent,1,n,'100%',cl)
this.tab.className='dcv-tab';}
wn.widgets.DocColumnView.prototype.refresh=function(){this.columns[this.items[0]].run();}
wn.widgets.dcvList=function(dcv,parent,doctype,opts){this.dcv=dcv;this.doctype=doctype;this.opts=opts;this.dtl=get_doctype_label(doctype);this.make_body(parent);this.selected_item=null;}
wn.widgets.dcvList.prototype.make_body=function(parent){this.make_toolbar(parent);this.make_search(parent);this.make_message(parent);this.make_list(parent);this.clear();}
wn.widgets.dcvList.prototype.make_toolbar=function(parent){var me=this;this.head=$a(parent,'div','list2-head');$gr(this.head,'#EEE','#CCC');var t=make_table(this.head,1,2,'100%',['60%','40%'],{verticalAlign:'middle'});var span=$a($td(t,0,0),'span','',{cssFloat:'left'},this.dtl);var refresh_icon=$a($td(t,0,0),'div','wn-icon ic-playback_reload',{marginLeft:'7px',cssFloat:'left'});refresh_icon.onclick=function(){me.run();}
this.btn=$btn($td(t,0,1),'+ New',function(){me.make_new();},{fontWeight:'bold',cssFloat:'right'},'green');}
wn.widgets.dcvList.prototype.make_search=function(parent){var me=this;this.searchbar=$a(parent,'div','list2-search');this.search_inp=$a_input(this.searchbar,'text');this.search_btn=$a(this.searchbar,'img','',{cursor:'pointer',marginLeft:'8px',marginBottom:'-3px'});this.search_btn.src='images/icons/magnifier.png';this.search_btn.onclick=function(){me.run();}}
wn.widgets.dcvList.prototype.make_message=function(parent){this.clear_message=$a(parent,'div','help_box',{margin:'4px',display:'none'},(this.opts.filter_by?('Select '+get_doctype_label(this.opts.filter_by[0])+' to see list'):''));this.no_result_message=$a(parent,'div','help_box',{margin:'4px',display:'none'},'No '+this.dtl+' created yet!');}
wn.widgets.dcvList.prototype.make_new=function(){var me=this;newdoc(this.doctype,function(dn){if(me.opts.filter_by){var val=me.dcv.columns[me.opts.filter_by[0]].get_selected();if(val)
locals[me.doctype][dn][me.opts.filter_by[1]]=val;}
if(me.opts.set_values){for(key in me.opts.set_values){locals[me.doctype][dn][key]=me.opts.set_values[key]}}});}
wn.widgets.dcvList.prototype.clear=function(){$dh(this.lst_area);$ds(this.clear_message)
$dh(this.no_result_message);this.clear_next();}
wn.widgets.dcvList.prototype.show_list=function(){$ds(this.lst_area);$dh(this.clear_message)
$dh(this.no_result_message);}
wn.widgets.dcvList.prototype.show_no_result=function(){if(!this.search_inp.value){$dh(this.lst_area);$dh(this.clear_message);$ds(this.no_result_message);}}
wn.widgets.dcvList.prototype.clear_next=function(){if(this.opts.next_col&&this.dcv.columns[this.opts.next_col])this.dcv.columns[this.opts.next_col].clear();}
wn.widgets.dcvList.prototype.make_list=function(parent){var me=this;this.lst_area=$a(parent,'div','list2-list-area');this.lst=new Listing('Profiles',1);this.lst.opts.hide_refresh=1;this.lst.opts.cell_style={padding:'0px'};this.lst.colwidths=['100%'];this.lst.get_query=function(){var q=me.build_query();this.query=q[0];this.query_max=q[1];}
this.lst.make(this.lst_area);this.lst.show_cell=function(cell,ri,ci,d){new wn.widgets.dcvListItem(cell,d[ri],me);}
this.lst.onrun=function(){me.show_list();me.clear_next();if(!me.lst.has_data())me.show_no_result();}}
wn.widgets.dcvList.prototype.run=function(){$dh(this.lst.results);this.lst.run();}
wn.widgets.dcvList.prototype.build_query=function(){var args={fields:this.opts.show_fields.join(', '),doctype:this.doctype,cond:''}
...
wn.widgets.dcvList.prototype.build_search_conditions=function(){var cl=new Array();if(this.opts.conditions){for(var i=0;i<this.opts.conditions.length;i++)cl.push(this.opts.conditions);}
...
return cl;}
wn.widgets.dcvList.prototype.add_filter_condition=function(cl){if(this.opts.filter_by){cl.push(repl('`%(filter)s` = "%(val)s"',{filter:this.opts.filter_by[1],val:this.dcv.columns[this.opts.filter_by[0]].get_selected()}));}
return cl;}
wn.widgets.dcvList.prototype.get_selected=function(){if(this.selected_item)return this.selected_item.det[0];else return'';}
wn.widgets.dcvListItem=function(cell,det,list2){this.det=det;this.list2=list2;this.make_body(cell);this.show_text();this.show_more_info();}
wn.widgets.dcvListItem.prototype.make_body=function(cell){var me=this;this.body=$a(cell,'div','list2-item-div')
...
wn.widgets.dcvListItem.prototype.make_with_icon=function(){var t=make_table(this.body,1,2,'100%',['','18px'])
$y($td(t,0,1),{verticalAlign:'middle'})
var img=$a($td(t,0,1),'img');img.src='images/icons/control_play.png';this.content=$td(t,0,0);}
wn.widgets.dcvListItem.prototype.show_text=function(){var me=this;this.label=$a(this.content,'div','list2-item-title','',this.det[0]);var span=$a(this.label,'span','link_type list2-edit-link','','[Edit]');span.onclick=function(){loaddoc(me.list2.doctype,me.det[0]);}}
wn.widgets.dcvListItem.prototype.show_more_info=function(){var det=this.det;if(det.length>1){var l=[];for(var i=1;i<det.length;i++){if(det[i]&&det[i]!=det[0])l.push(det[i]);}
if(l.length)
this.more_info=$a(this.content,'div','list2-item-more-info','',l.join(', '))}}
wn.widgets.dcvListItem.prototype.select=function(){if(this.list2.selected_item)this.list2.selected_item.deselect();this.body.className='list2-item-div list2-item-selected';this.list2.selected_item=this;}
wn.widgets.dcvListItem.prototype.deselect=function(){this.body.className='list2-item-div';}
...
this.make_items=function(){for(var i=0;i<this.opts.items.length;i++){var item=this.opts.items[i];if((item.display&&item.display())||!item.display){var div=$a(this.wrapper,'div','psidebar-section-item');this.make_one_item(item,div);}}}
...
this.refresh=function(){this.make_head();if(this.opts.render){this.opts.render(this.wrapper);}
else
this.make_items();}
this.refresh();}
...
this.invite_link=$ln(this.wrapper,'Invite to follow',function(ln){me.invite()},{marginLeft:'8px',fontSize:'11px'});this.help_link=$ln(this.wrapper,'[?]',function(ln){msgprint('<b>What is follow?</b> When you follow something, you are kept updated \
    with all the events that happen around it. Much like Twitter or Quora')},{marginLeft:'8px',fontSize:'11px'});this.show_followers();}
...
this.invite=function(){if(!this.dialog){this.dialog=new wn.widgets.Dialog({title:'Invite someone to follow',width:400,fields:[{fieldtype:'Link',fieldname:'user',label:'Select the user to who you want to invite',options:'Profile',reqd:1},{fieldtype:'Button',fieldname:'invite',label:'Invite'}]});this.dialog.make()
this.dialog.fields_dict.user.get_query=function(){return'SELECT tabProfile.name, tabProfile.first_name, tabProfile.last_name FROM tabProfile WHERE tabProfile.name LIKE "%s" OR tabProfile.first_name LIKE "%s" or tabProfile.last_name LIKE "%s" ORDER BY tabProfile.name ASC LIMIT 50'}
this.dialog.fields_dict.invite.onclick=function(){var v=me.get_values();if(v){var btn=me.dialog.fields_dict.invite;btn.set_working();$c('webnotes.widgets.follow.follow',{dt:me.dt,dn:me.dn,user:v.user},function(r,rt){btn.done_working();me.update_follow(r);})}}}
this.dialog.show();}

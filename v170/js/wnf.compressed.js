...
function $c(command,args,fn,on_timeout,no_spinner,freeze_msg){var req=newHttpReq();ret_fn=function(){if(checkResponse(req,on_timeout,no_spinner,freeze_msg)){if(!no_spinner)hide_loading();var rtxt=req.responseText;try{var r=eval("var a="+rtxt+";a");}catch(e){msgprint('Handler Exception:'+rtxt);return;}
...
else if(d[0]=='Select'){c1.innerHTML=d[1];this.widgets[d[1]]=new SelectWidget(c2,[],'160px');if(d[2])$a(c2,'div','comment').innerHTML=d[2];}
...
cur_dialog.hide();}});list_opts={cell_style:{padding:'3px 2px'},alt_cell_style:{backgroundColor:'#F2F2FF'},head_style:{height:'20px',overflow:'hidden',verticalAlign:'middle',textAlign:'center',fontWeight:'bold',padding:'1px',fontSize:'13px'},head_main_style:{padding:'0px'},hide_export:0,hide_print:0,hide_refresh:0,hide_rec_label:0,show_calc:1,show_empty_tab:1,show_bottom_paging:1,round_corners:1,no_border:0,append_records:0};function Listing(head_text,no_index,no_loading){this.start=0;this.page_len=20;this.paging_len=5;this.filters_per_line=3;this.cell_idx=0;this.head_text=head_text?head_text:'Result';this.keyword='records';this.no_index=no_index;this.underline=1;this.show_cell=null;this.show_result=null;this.colnames=null;this.colwidths=null;this.coltypes=null;this.coloptions=null;this.filters={};this.sort_list={};this.sort_order_dict={};this.sort_heads={};this.is_std_query=false;this.server_call=null;this.no_loading=no_loading;this.opts=copy_dict(list_opts);}
...
this.filters={};}
$ds(this.filter_wrapper);if((!this.inp_tab)||(this.cell_idx==this.filters_per_line)){this.inp_tab=$a(this.filter_area.div,'table','',{width:'100%'});this.inp_tab.insertRow(0);for(var i=0;i<this.filters_per_line;i++){this.inp_tab.rows[0].insertCell(i);}
this.cell_idx=0;}
var c=this.inp_tab.rows[0].cells[this.cell_idx];this.cell_idx++;$y(c,{width:cint(100/this.filters_per_line)+'%',textAlign:'left',verticalAlign:'top'});var d1=$a(c,'div');d1.innerHTML=label;$y(d1,{marginBottom:'2px'});var d2=$a(c,'div');if(in_list(['Text','Small Text','Code','Text Editor'],ftype))
...
MenuToolbar.prototype.add_top_menu=function(label,onclick,add_icon){var li=$a(this.ul,'li');$y(li,{marginRight:'8px'});var a=$a(li,'div',this.top_menu_style);var a2=$a(a,'span','',{textDecoration:'underline'});a2.innerHTML=label;if(add_icon){var img=$a(a,'img','',{});img.src=add_icon;$y(a,{marginLeft:'0px'});}
li.label=a;var me=this;a.onclick=function(){onclick();};a.onmouseover=function(){this.set_selected();if(this.my_mouseover)this.my_mouseover(this);}
a.onmouseout=function(){if(this.my_mouseout)this.my_mouseout(this);this.set_unselected();}
...
PageHeader.prototype.add_button=function(label,fn,bold,icon){var btn=$a(this.toolbar_area,'button');btn.innerHTML=label;btn.onclick=fn;if(bold)$y(btn,{fontWeight:'bold'});$(btn).button({icons:{primary:icon}});this.show_toolbar();}
PageHeader.prototype.show_toolbar=function(){$dh(this.separator);$ds(this.toolbar_area);}
...
this.wrapper=document.createElement('div');if(!this.with_label){this.label_area=$a(this.wrapper,'div');if(this.in_grid){$dh(this.label_area);}
this.input_area=$a(this.wrapper,'div');this.disp_area=$a(this.wrapper,'div');this.help_icon=null;}else{var t=$a(this.wrapper,'table','frm_field_table');var r=t.insertRow(0);this.r=r;var lc=r.insertCell(0);this.input_cell=r.insertCell(1);lc.className='datalabelcell';this.input_cell.className='datainputcell';var lt=make_table($a(lc,'div'),1,3,'100%',[null,'20px','20px']);this.label_icon=$a($td(lt,0,2),'img');$dh(this.label_icon);this.label_icon.src='images/icons/error.gif';this.help_icon=$a($td(lt,0,1),'img','',{cursor:'pointer'});$dh(this.help_icon);this.help_icon.src='images/icons/help.gif';this.label_cell=$td(lt,0,0);this.input_area=$a(this.input_cell,'div','input_area');this.disp_area=$a(this.input_cell,'div');}
...
Field.prototype.set_comment=function(){var me=this;if(!this.help_icon)return;if(this.df.description){$ds(this.help_icon);this.help_icon.onclick=function(){msgprint(me.df.description)};}else{$dh(this.help_icon);}}
...
function makeinput_popup(me,iconsrc,iconsrc1,iconsrc2){me.input=$a(me.input_area,'div');me.input.onchange=function(){}
...
me.btn.setAttribute('title','Select Date');me.btn.style.margin='4px 2px 2px 8px';if(iconsrc1){var c2=tab.rows[0].insertCell(2);$w(c1,'18px');me.btn1=$a(c2,'img','btn-img');me.btn1.src=iconsrc1;me.btn1.setAttribute('title','Open Link');me.btn1.style.margin='4px 2px 2px 0px';}
if(iconsrc2){$w(c2,'18px');me.btn2=$a(tab.rows[0].insertCell(3),'img','btn-img');me.btn2.src=iconsrc2;me.btn2.setAttribute('title','Create New');me.btn2.style.margin='4px 2px 2px 0px';$dh(me.btn2);}
if(me.df.colour)
...
var about_dialog;function WNToolbar(parent){var me=this;this.setup=function(){this.wrapper=$a(parent,'div','',{borderBottom:'1px solid #CDF'});this.body_tab=make_table(this.wrapper,1,3,'100%',['55%','20%','25%'],{padding:'2px'});$y($td(this.body_tab,0,1),{paddingTop:'3px',paddingBottom:'0px'});this.model_tab=make_table($td(this.body_tab,0,1),1,4,null,['140px','140px','140px'],{padding:'2px'});this.menu=new MenuToolbar($td(this.body_tab,0,0));this.setup_home();this.setup_new();this.setup_recent();this.setup_options();this.setup_help();this.setup_report_builder();this.setup_logout();this.setup_search();}
this.setup_options=function(){var tm=this.menu.add_top_menu('Options',function(){},"images/ui/down-arrow1.gif");var fn=function(){if(this.dt=='Page')
...
this.setup_home=function(){this.menu.add_top_menu('<b>Home</b>',function(){loadpage(home_page);});}
this.setup_recent=function(){this.rdocs=me.menu.add_top_menu('Recent',function(){},"images/ui/down-arrow1.gif");this.rdocs.items={};var fn=function(){loaddoc(this.dt,this.dn);mclose();}
...
this.setup_new=function(){me.menu.add_top_menu('Create New...',function(){me.show_new();});me.show_new=function(){if(!me.new_dialog){var d=new Dialog(240,140,"Create a new record");d.make_body([['HTML','Select'],['Button','Go',function(){me.new_dialog.hide();new_doc(me.new_sel.inp.value);}]]);me.new_dialog=d;me.new_sel=new SelectWidget(d.widgets['Select'],profile.can_create.sort(),'200px');}
me.new_dialog.show();}}
this.setup_report_builder=function(){me.menu.add_top_menu('Report Builder...',function(){me.show_rb();});me.show_rb=function(){if(!me.rb_dialog){var d=new Dialog(240,140,"Build a report for");d.make_body([['HTML','Select'],['Button','Go',function(){me.rb_dialog.hide();loadreport(me.rb_sel.inp.value,null,null,1);}]]);me.rb_dialog=d;me.rb_sel=new SelectWidget(d.widgets['Select'],profile.can_get_report.sort(),'200px');}
me.rb_dialog.show();}}
this.setup_search=function(){$y(page_body.search_area,{padding:'8px 0px'});var d=$a(page_body.search_area,'div','',{cssFloat:'right'});var t=make_table(d,1,3,null,['18px','130px','60px'],{padding:'2px',verticalAlign:'middle',textAlign:'right'});var img=$a($td(t,0,0),'img');img.src='images/icons/magnifier.gif';this.search_sel=new SelectWidget($td(t,0,1),[],'120px');this.search_sel.inp.value='Search...';$y($td(this.model_tab,0,3),{paddingTop:'0px'});function open_quick_search(){if(me.search_sel.inp.value)
selector.set_search(me.search_sel.inp.value);me.search_sel.disabled=1;selector.show();}
me.search_sel.set_options(profile.can_read.sort());me.search_sel.onchange=function(){open_quick_search();this.value='Search...';}
me.search_btn=$a($td(t,0,2),'button')
$(me.search_btn).html('Search');me.search_btn.onclick=function(){open_quick_search();this.value='Search...';}
...
function loaddocbrowser(dt,label,fields){var show=function(){doc_browser.show(dt,label,fields);nav_obj.open_notify('DocBrowser',dt,'');}
if(doc_browser)
show();else
...
var callback=function(r,rt){if(r.exc)msgprint(r.ext);setup_globals(r);setup_history(r);setup_events();var a=new Body();page_body.run_startup_code();page_body.setup_sidebar_menu();for(var i=0;i<startup_list.length;i++){startup_list[i]();}

...
$c('webnotes.widgets.form.validate_link',{'value':me.txt.value,'options':me.df.options},function(r,rt){if(selector&&selector.display)return;if(r.message=='Ok'){me.run_trigger();}else{var astr='';if(in_list(profile.can_create,me.df.options))astr=repl('<br><br><span class="link_type" onclick="newdoc(\'%(dt)s\')">Click here</span> to create a new %(dt)s',{dt:me.df.options})
msgprint(repl('error:<b>%(val)s</b> is not a valid %(dt)s.<br><br>You must first create a new %(dt)s <b>%(val)s</b> and then select its value. To find an existing %(dt)s, click on the magnifying glass next to the field.%(add)s',{val:me.txt.value,dt:me.df.options,add:astr}));me.txt.value='';me.set('');}});}}}

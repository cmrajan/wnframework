...
function in_list(list,item){return list.indexOf(item)==-1?false:true;}
...
$wid_active=function(ele){if(ele.disabled)return;$y(ele,{border:'1px solid #000',color:'#444'});$gr(ele,'#FFF','#EEE');if(ele.no_left_border)$y(ele,{borderLeft:'0px'})
...
$item_normal=function(ele){$y(ele,{padding:'6px 8px',cursor:'pointer',marginRight:'8px',fontWeight:'normal',whiteSpace:'nowrap',overflow:'hidden',borderBottom:'1px solid #DDD'});$bg(ele,'#FFF');$fg(ele,'#000');}
...
$item_set_working=function(ele){if(ele.loading_img){$di(ele.loading_img)}else{ele.loading_img=$a(ele,'img','',{marginLeft:'4px',marginBottom:'-2px'});ele.loading_img.src='images/ui/button-load.gif';}}
$item_done_working=function(ele){if(ele.loading_img){$dh(ele.loading_img)};}
...
if(this.opts.show_report){make_btn('Report Builder','ui-icon-clipboard',function(){loadreport(me.dt);},0);}
...
for(var ci=0;ci<nc;ci++){var c=$td(this.result_tab,ri,ci+(this.no_index?0:1));if(c){c.innerHTML='';if(this.show_cell)this.show_cell(c,ri-start,ci,d);else this.std_cell(d,ri-start,ci);}}}}}}
Listing.prototype.refresh_more_button=function(nr){var me=this;if((this.start+nr)==this.max_len){if(this.more_btn)$dh(this.more_btn);}else{if(!this.more_btn){$y(this.bottom_div,{margin:'8px 0px 16px 0px',textAlign:'center'});this.more_btn=$btn(this.bottom_div,'Show more results...',function(){me.start=me.start+me.page_len;me.more_btn.set_working();me.run(1,function(){me.more_btn.done_working();});},{fontSize:'14px'},0,1);$y(this.more_btn.loading_img,{marginBottom:'0px'});}
$di(this.more_btn);}}
...
var def_ph_style={wrapper:{marginBottom:'16px',backgroundColor:'#EEE'},main_heading:{fontSize:'18px',padding:'4px 0px',fontWeight:'bold'},sub_heading:{fontSize:'14px',marginBottom:'8px',color:'#555',display:'none'},separator:{borderTop:'3px solid #444'},toolbar_area:{padding:'3px 0px',display:'none',borderBottom:'1px solid #AAA'}}
...
this.lhs=$td(this.t1,0,0);this.main_head=$a(this.lhs,'div','',def_ph_style.main_heading);this.sub_head=$a(this.lhs,'div','',def_ph_style.sub_heading);this.separator=$a(this.wrapper,'div','',def_ph_style.separator);this.toolbar_area=$a(this.wrapper,'div','',def_ph_style.toolbar_area);this.padding_area=$a(this.wrapper,'div','',{padding:'3px'});$y($td(this.t1,0,1),{textAlign:'right',padding:'3px'});this.close_btn=$btn($td(this.t1,0,1),'Close',function(){nav_obj.show_last_open();},0);if(main_text)this.main_head.innerHTML=main_text;if(sub_text)this.sub_head.innerHTML=sub_text;this.buttons={};this.buttons2={};}
...
$(document).ready(function(){jQuery.fn.redswap=function(options){var selectedItems=this;redsettings=jQuery.extend({speed:"Medium",opacity:0.7},options);$(selectedItems).mouseover(function(){$(selectedItems).disableSelection();$(selectedItems).droppable({drop:function(event,ui){dropindex=$(selectedItems).index(this);var dragposition=$(selectedItems[dragindex]).position();var dropposition=$(selectedItems[dropindex]).position();withDifference=parseInt(dragposition.left)-parseInt(dropposition.left);heightDifference=parseInt(dragposition.top)-parseInt(dropposition.top);$(selectedItems[dropindex]).animate({left:'+='+withDifference,top:'+='+heightDifference},redsettings.speed,function(){});$(selectedItems[dragindex]).animate({left:'+='+(withDifference*(-1)),top:'+='+(heightDifference*(-1))},redsettings.speed,function(){});var a=selectedItems[dropindex].idx;selectedItems[dropindex].idx=selectedItems[dragindex].idx
selectedItems[dragindex].idx=a;},});$(this).draggable({opacity:redsettings.opacity,helper:'clone',containment:'parent',scroll:false,cursor:'move',drag:function(event,ui){dragindex=$(selectedItems).index(this);}});});};});var no_value_fields=['Section Break','Column Break','HTML','Table','FlexTable','Button','Image'];var codeid=0;var code_editors={};function Field(){}
...
return v;};var _last_link_value=null;function LinkField(){}LinkField.prototype=new Field();LinkField.prototype.with_label=1;LinkField.prototype.make_input=function(){var me=this;if(me.df.no_buttons){this.txt=$a(this.input_area,'input');this.input=this.txt;}else{makeinput_popup(this,'ic-zoom','ic-sq_next','ic-sq_plus');me.setup_buttons();me.onrefresh=function(){if(me.can_create&&cur_frm.doc.docstatus==0)$ds(me.btn2);else $dh(me.btn2);}}
...
fetch=cur_frm.fetch_dict[me.df.fieldname].columns.join(', ');$c('webnotes.widgets.form.validate_link',{'value':me.txt.value,'options':me.df.options,'fetch':fetch},function(r,rt){if(selector&&selector.display)return;if(r.message=='Ok'){if(r.fetch_values)me.set_fetch_values(r.fetch_values);me.run_trigger();}else{var astr='';if(in_list(profile.can_create,me.df.options))astr=repl('<br><br><span class="link_type" onclick="newdoc(\'%(dt)s\')">Click here</span> to create a new %(dtl)s',{dt:me.df.options,dtl:get_doctype_label(me.df.options)})
...
var show_doc=function(r,rt){if(locals[doctype]&&locals[doctype][name]){page_body.set_status('Done');var frm=frms[doctype];if(menuitem)frm.menuitem=menuitem;if(onload)onload(frm);if(r&&r.no_of_comments)frm.n_comments[name]=r.no_of_comments;frm.refresh(name);if(!frm.in_dialog)
...
_tags={dialog:null,color_map:null,all_tags:[],colors:{'Default':'#489','Red':'#FF0000','Blue':'#000088','Green':'#008800','Orange':'#FF8800'},color_list:['Default','Red','Blue','Green','Orange']}
ItemBrowserPage=function(){this.lists={};this.dt_details={};this.cur_list=null;this.my_page=page_body.add_page('ItemBrowser');this.wrapper=$a(this.my_page,'div');var h=$a(this.wrapper,'div');this.body=$a(this.wrapper,'div');}
ItemBrowserPage.prototype.show=function(dt,label,field_list){var me=this;if(this.cur_list)$dh(this.cur_list.wrapper);if(!me.lists[dt]){me.lists[dt]=new ItemBrowser(me.body,dt,label,field_list);}
me.cur_list=me.lists[dt];me.cur_list.show();page_body.change_to('ItemBrowser');}
ItemBrowser=function(parent,dt,label,field_list){var me=this;this.label=label?label:dt;this.dt=dt;this.field_list=field_list;this.tag_filter_dict={};this.items=[];this.wrapper=$a(parent,'div','',{display:'none'});this.head=$a(this.wrapper,'div');var l=get_doctype_label(dt);l=(l.toLowerCase().substr(-4)=='list')?l:(l+' List')
this.page_head=new PageHeader(this.head,l);$y(this.page_head.wrapper,{marginBottom:'0px'});$dh(this.page_head.separator);this.no_result_area=$a(this.wrapper,'div');this.no_result_message=$a(this.no_result_area,'span','',{backgroundColor:'#FFC',padding:'6px'});this.loading_div=$a(this.wrapper,'div','',{margin:'200px 0px',textAlign:'center',fontSize:'14px',color:'#888',display:'none'});this.loading_div.innerHTML='Loading...';this.body=$a(this.wrapper,'div');this.toolbar_area=$a(this.body,'div','',{padding:'4px',backgroundColor:'#EEE',borderTop:'1px solid #CCC',borderBottom:'1px solid #CCC'});this.sub_toolbar=$a(this.body,'div','',{marginBottom:'8px',padding:'4px',textAlign:'right',fontSize:'11px',color:'#444'});this.archives_label=$a(this.body,'div','',{margin:'8px 0px',padding:'4px',fontSize:'14px',textAlign:'center',color:'#444',display:'none',backgroundColor:'#FFD'});this.archives_label.innerHTML='<i>Showing from Archives</i>';this.trend_area=$a(this.body,'div','',{marginBottom:'16px',padding:'4px',backgroundColor:'#EEF',border:'1px solid #CCF',display:'none'});$br(this.trend_area,'5px');this.tag_filters=$a(this.body,'div','',{marginBottom:'8px',display:'none',padding:'6px 8px 8px 8px',backgroundColor:'#FFD'});var span=$a(this.tag_filters,'span','',{marginRight:'4px',color:'#444'});span.innerHTML='<i>Showing for:</i>';this.tag_area=$a(this.tag_filters,'span');}
...
$c_obj('Menu Control','get_trend',this.dt,callback);me.trend_btn.set_working();}else{$(this.trend_area).slideDown();me.trend_btn.innerHTML='Hide Activity';me.trend_on=1;}}}
ItemBrowser.prototype.show=function(show_callback){var me=this;var callback=function(r,rt){if(r.message=='Yes'){if(!me.loaded)
me.load_details(show_callback);else
me.show_results();if(show_callback)show_callback();}else{me.show_no_result();if(show_callback)show_callback();}}
...
if(r.message.color_map){_tags.color_map=r.message.color_map;}}
var with_color_map=0;if(!_tags.color_map)with_color_map=1;var fl=this.field_list?this.field_list.split('\n'):'';$c_obj('Menu Control','get_dt_details',JSON.stringify([this.dt,fl,with_color_map]),callback);this.loaded=1;}
ItemBrowser.prototype.show_results=function(){$ds(this.wrapper);$dh(this.loading_div);$ds(this.body);$dh(this.no_result_area);set_title(get_doctype_label(this.label));}
...
ItemBrowser.prototype.show_no_result=function(){$ds(this.wrapper);$dh(this.loading_div);$dh(this.body);$ds(this.no_result_area);this.no_result_message.innerHTML=repl('No %(dt)s records found. <span class="link_type" onclick="newdoc(\'%(dt)s\')">Click here</span> to create your first %(dt)s',{dt:get_doctype_label(this.dt)});set_title(get_doctype_label(this.label));}
...
fl.push(q.table+'.docstatus');q.fields=fl.join(', ');q.conds=q.table+'.docstatus < '+((me.show_cancelled&&me.show_cancelled.checked)?3:2)+' ';me.add_tag_conditions(q);me.add_search_conditions(q);this.query=repl("SELECT %(fields)s FROM %(table)s WHERE %(conds)s",q);this.query_max=repl("SELECT COUNT(*) FROM %(table)s WHERE %(conds)s",q);}
...
ItemBrowser.prototype.archive_items=function(){var me=this;arg={'action':this.show_archives.checked?'Restore':'Archive','items':this.get_checked()}
$c_obj('Menu Control','archive_items',JSON.stringify(arg),function(r,rt){if(!r.exc)me.run();})}
function ItemBrowserItem(parent,det,ib){this.wrapper=$a(parent,'div');$y(this.wrapper,{borderTop:'1px solid #DDD'});this.tab=make_table(this.wrapper,1,2,'100%',[(200/7)+'%',null]);this.body=$a($td(this.tab,0,1),'div');this.det=det;this.ib=ib;this.dn=det[0];this.make_check();this.make_details();this.make_tags();this.add_timestamp();}
...
ItemBrowserItem.prototype.make_details=function(){var me=this;var div=$a(this.body,'div');var span=$a($td(this.tab,0,0),'span','link_type',{fontWeight:'bold'});span.innerHTML=me.dn;span.onclick=function(){loaddoc(me.ib.dt,me.dn,null,null,(me.ib.show_archives?me.ib.show_archives.checked:null));}
var tmp=[];var cl=me.ib.dt_details.columns;var first_property=1;for(var i=3;i<me.det.length;i++){if(cl[i]&&cl[i][1]&&me.det[i]){if(cl[i][1].indexOf('Status')!=-1||cl[i][1].indexOf('Group')!=-1||cl[i][1].indexOf('Priority')!=-1||cl[i][1].indexOf('Type')!=-1){me.add_tag(me.det[i],1,cl[i][0]);}else{if(!first_property){var span=$a(div,'span');span.innerHTML=',';}else first_property=0;var span=$a(div,'span','',{color:'#888'});span.innerHTML=' '+cl[i][1]+': ';var span=$a(div,'span');$s(span,me.det[i],(cl[i][2]=='Link'?'Data':cl[i][2]),cl[i][3]);}}}}
ItemBrowserItem.prototype.make_tags=function(){var docstatus=cint(this.det[this.det.length-1]);if(this.ib.dt_details.submittable)
{this.add_tag((docstatus?'Submitted':'Draft'),1,'docstatus');}
if(this.det[2]){var tl=this.det[2].split(',');for(var i=0;i<tl.length;i++)if(tl[i])this.add_tag(tl[i],0,'_user_tags');}
this.make_add_tag();}
...
ItemBrowserItem.prototype.make_tag_area=function(){var div=$a(this.body,'div','',{margin:'3px 0px',padding:'3px 0px'});this.tag_area=$a(div,'span','',{marginRight:'4px'});this.add_tag_area=$a(div,'span');this.tag_list=[];}
ItemBrowserItem.prototype.add_tag=function(label,static,fieldname){var me=this;if(!this.tag_area)this.make_tag_area();if(in_list(this.tag_list,label))return;var tag=new ItemBrowserTag(this.tag_area,label,this.ib.dt,this.dn,static);tag.remove=function(tag_ref){var callback=function(r,rt){$dh(tag_ref.body);var nl=[];for(var i in me.tag_list)if(me.tag_list[i]!=tag_ref.label)nl.push(me.tag_list[i]);me.tag_list=nl;}
...
ib.run();}
me.ib.tag_filter_dict[tag_ref.label]=filter_tag;$ds(me.ib.tag_filters);me.ib.run();}}
ItemBrowserItem.prototype.make_color_picker=function(dialog){var n_cols=_tags.color_list.length;var div=$a(dialog.widgets['Tag'],'div','',{margin:'8px 0px'});div.tab=make_table(div,2,n_cols,(26*n_cols)+'px',[],{textAlign:'center'});div.pickers=[];for(var i=0;i<n_cols;i++){var wrapper=$a($td(div.tab,0,i),'div','',{margin:'5px',border:'3px solid #FFF'})
...
function ItemBrowserTag(parent,label,dt,dn,static){this.dt=dt;this.dn=dn;this.label=label;var me=this;var bg=_tags.colors[(_tags.color_map?(_tags.color_map[label]?_tags.color_map[label]:'Default'):'Default')];if(label=='Submitted')bg='#459';if(label=='Draft')bg='#4A5';this.body=$a(parent,'span','',{padding:'2px 4px',backgroundColor:bg,color:'#FFF',marginRight:'4px',fontSize:'11px',cursor:'pointer'});$(this.body).hover(function(){$op(this,60);},function(){$op(this,100);});this.refresh_color=function(){bg=_tags.color_map?(_tags.color_map[this.label]?_tags.color_map[this.label]:'Default'):'Default';$y(this.body,{backgroundColor:_tags.colors[bg]});}
$br(this.body,'3px');var span=$a(this.body,'span');span.innerHTML=label;span.onclick=function(){if(me.onclick)me.onclick(me);}

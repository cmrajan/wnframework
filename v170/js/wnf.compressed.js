...
function $a(parent,newtag,className,cs,innerHTML){if(parent&&parent.substr)parent=$i(parent);var c=document.createElement(newtag);if(parent)
parent.appendChild(c);if(className)c.className=className;if(cs)$y(c,cs);if(innerHTML)c.innerHTML=innerHTML;return c;}
...
TagList=function(parent,start_list,dt,dn,static,onclick){this.start_list=start_list;this.tag_list=[];this.dt=dt;this.onclick=onclick;this.dn=dn;this.static;this.parent=parent;this.make_body();}
...
d.tag_input.txt.value='';d.hide();if(!r.message)return;me.add_tag(r.message,0,'_user_tags');}
...
return ret;}
Field.prototype.refresh_mandatory=function(){if(this.not_in_form)return;if(this.label_area){if(this.df.reqd){this.label_area.style.color="#d22";if(this.txt)$y(this.txt,{backgroundColor:"#FEE"});else if(this.input)$y(this.input,{backgroundColor:"#FEE"});}else{this.label_area.style.color="#222";if(this.txt)$y(this.txt,{backgroundColor:"#FFF"});else if(this.input)$y(this.input,{backgroundColor:"#FFF"});}}

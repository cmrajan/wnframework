...
if(this.opts.new_doctype){this.new_btn=$btn(this.toolbar_area,'New '+get_doctype_label(this.opts.new_doctype),function(){newdoc(me.opts.new_doctype);},{marginLeft:'7px'});}}
...
SingleTag.prototype.make_label=function(){var me=this;this.label_span=$a(this.body,'span','',null,this.label);this.label_span.onclick=function(){if(me.onclick)me.onclick(me);}}
...
Field.prototype.run_trigger=function(){this.refresh_label_icon();if(this.df.reqd&&!is_null(this.get_value())&&this.set_as_error)
...
this.page_head=new PageHeader(this.head,l);$y(this.page_head.wrapper,{marginBottom:'0px'});$dh(this.page_head.separator);this.no_result_area=$a(this.wrapper,'div','help_box',{fontSize:'14px',textAlign:'center'});this.loading_div=$a(this.wrapper,'div','',{margin:'200px 0px',textAlign:'center',fontSize:'14px',color:'#444',display:'none'});this.loading_div.innerHTML='Loading<img src="images/ui/button-load.gif" style="margin-bottom: -2px; margin-left: 8px">';this.body=$a(this.wrapper,'div');this.setup_toolbar();this.setup_sidebar();}
ItemBrowser.prototype.setup_sidebar=function(){var me=this;var tab=make_table(this.body,1,2,'100%',['75%','25%'],{paddingRight:'7px'});this.list_area=$td(tab,0,0);$y($td(tab,0,1),{paddingTop:'53px'});this.sidebar=new wn.widgets.PageSidebar($td(tab,0,1),{sections:[{title:'Top Tags',render:function(body){new wn.widgets.TagCloud(body,me.dt,function(tag){me.set_tag_filter(tag)});}}]});}
ItemBrowser.prototype.setup_toolbar=function(){this.toolbar_area=$a(this.body,'div','',{padding:'3px',backgroundColor:'#EEE'});$br(this.toolbar_area,'3px');$gr(this.toolbar_area,'#DDD','#CCC');this.sub_toolbar=$a(this.body,'div','',{marginBottom:'7px',padding:'3px',textAlign:'right',fontSize:'11px',color:'#444'});this.archives_label=$a(this.body,'div','help_box_big',{display:'none'});this.archives_label.innerHTML='Showing from Archives'
...
ItemBrowser.prototype.load_details=function(load_callback){var me=this;var callback=function(r,rt){me.dt_details=r.message;if(r.message){me.make_toolbar();me.make_the_list(me.dt,me.list_area);me.show_results();if(load_callback)load_callback();}}
...
ItemBrowser.prototype.add_tag_conditions=function(q){var me=this;if(keys(me.tag_filter_dict).length){var cl=[];for(var key in me.tag_filter_dict){var val=key;var op='=';var fn=me.tag_filter_dict[key].fieldname;fn=fn?fn:'_user_tags';if(fn=='docstatus')val=(key=='Draft'?'0':'1');if(fn=='_user_tags'){val='%,'+key+'%';op=' LIKE ';}
...
this.taglist=new TagList(div,tl,this.ib.dt,this.dn,0,function(tag){me.ib.set_tag_filter(tag);});if(this.ib.dt_details.submittable){this.taglist.add_tag((docstatus?'Submitted':'Draft'),1,'docstatus','#c0c0c0');}}

...
return s;}
function objpos(obj){if(obj.substr)obj=$i(obj);var acc_lefts=0;var acc_tops=0;if(!obj)show_alert("No Object Specified");var co={};while(obj){acc_lefts+=obj.offsetLeft;acc_tops+=obj.offsetTop;if(isIE){if(obj!=window.document.body){acc_tops-=obj.scrollTop;}}else{var op=obj.offsetParent
var scr_obj=obj;while(scr_obj&&(scr_obj!=op)&&(scr_obj!=window.document.body)){acc_tops-=scr_obj.scrollTop;scr_obj=scr_obj.parentNode;}}
...
function $c(command,args,fn,on_timeout,no_spinner,freeze_msg){var req=newHttpReq();ret_fn=function(){if(checkResponse(req,on_timeout,no_spinner,freeze_msg)){if(!no_spinner)hide_loading();var rtxt=req.responseText;var r=eval("var a="+rtxt+";a")
...
Dialog.prototype.show=function(){var d=get_screen_dims();this.wrapper.style.left=((d.w-this.w)/2)+'px';this.wrapper.style.top=(get_scroll_top()+((d.h-this.h)/2))+'px';top_index++;$y(this.wrapper,{zIndex:top_index});$ds(this.wrapper);freeze();this.display=true;cur_dialog=this;if(this.onshow)this.onshow();}
...
cur_dialog.hide();}});list_opts={cell_style:{padding:'3px 2px'},alt_cell_style:{backgroundColor:'#F2F2FF'},head_style:{height:'20px',overflow:'hidden',verticalAlign:'middle',textAlign:'center',fontWeight:'bold',padding:'1px',fontSize:'13px'},head_main_style:{padding:'0px'},hide_export:0,hide_print:0,hide_refresh:0,hide_rec_label:0,show_calc:1,show_empty_tab:1,show_bottom_paging:1,round_corners:1,no_border:0,append_records:0};function Listing(head_text,no_index,no_loading){this.start=0;this.page_len=20;this.paging_len=5;this.filters_per_line=3;this.head_text=head_text?head_text:'Result';this.keyword='records';this.no_index=no_index;this.underline=1;this.show_cell=null;this.show_result=null;this.colnames=null;this.colwidths=null;this.coltypes=null;this.coloptions=null;this.filters={};this.sort_list={};this.sort_order_dict={};this.sort_heads={};this.is_std_query=false;this.server_call=null;this.no_loading=no_loading;this.opts=copy_dict(list_opts);}
...
this.make_result_tab();this.bottom_div=$a(this.body_area,'div','',{paddingTop:'8px',height:'22px'});var make_btn=function(label,icon,onclick,bold){var btn=$a(me.btn_area,'button');if(bold)$y(btn,{fontWeight:'bold'});btn.innerHTML=label;btn.onclick=onclick;}
...
me.refresh(r.values.length,nc,r.values);me.total_records=r.n_values;me.set_rec_label(r.n_values,r.values.length);}else{if(!me.opts.append_records){me.n_records=0;me.set_rec_label(0);if(!me.show_empty_tab){me.remove_result_tab();me.make_result_tab(0);}else{me.clear_tab();}}}
...
MenuToolbar.prototype.add_top_menu=function(label,onclick){var li=$a(this.ul,'li');var a=$a(li,'a',this.top_menu_style);li.label=a;var me=this;a.onclick=function(){onclick();};a.innerHTML=label;a.onmouseover=function(){this.set_selected();if(a.my_mouseover)a.my_mouseover(this);}
a.onmouseout=function(){if(a.my_mouseout)a.my_mouseout(this);this.set_unselected();}
a.set_unselected=function(){if(this.dropdown&&this.dropdown.is_active)
return;this.className=me.top_menu_style;me.is_active=0;}
...
MenuToolbar.prototype.make_dropdown=function(tm){var me=this;tm.dropdown=new DropdownMenu(tm.parentNode,this.dropdown_width);tm.my_mouseover=function(){this.dropdown.show();}
...
_c.popup_list=[];_c.cal_displayed=0;_c.MONTH_NAMES=new Array('January','February','March','April','May','June','July','August','September','October','November','December','Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec');_c.DAY_NAMES=new Array('Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sun','Mon','Tue','Wed','Thu','Fri','Sat');_c.LZ=function(x){return(x<0||x>9?"":"0")+x}
_c.formatDate=function(date,format){format=format+"";var result="";var i_format=0;var c="";var token="";var y=date.getYear()+"";var M=date.getMonth()+1;var d=date.getDate();var E=date.getDay();var H=date.getHours();var m=date.getMinutes();var s=date.getSeconds();var yyyy,yy,MMM,MM,dd,hh,h,mm,ss,ampm,HH,H,KK,K,kk,k;var value=new Object();if(y.length<4){y=""+(y-0+1900);}
value["y"]=""+y;value["yyyy"]=y;value["yy"]=y.substring(2,4);value["M"]=M;value["MM"]=_c.LZ(M);value["MMM"]=_c.MONTH_NAMES[M-1];value["NNN"]=_c.MONTH_NAMES[M+11];value["d"]=d;value["dd"]=_c.LZ(d);while(i_format<format.length){c=format.charAt(i_format);token="";while((format.charAt(i_format)==c)&&(i_format<format.length)){token+=format.charAt(i_format++);}
if(value[token]!=null){result=result+value[token];}
else{result=result+token;}}
return result;}
_c._getInt=function(str,i,minlength,maxlength){function _isInteger(val){var digits="1234567890";for(var i=0;i<val.length;i++){if(digits.indexOf(val.charAt(i))==-1){return false;}}
return true;}
for(var x=maxlength;x>=minlength;x--){var token=str.substring(i,i+x);if(token.length<minlength){return null;}
if(_isInteger(token)){return token;}}
return null;}
_c.getDateFromFormat=function(val,format){val=val+"";format=format+"";var i_val=0;var i_format=0;var c="";var token="";var token2="";var x,y;var now=new Date();var year=now.getYear();var month=now.getMonth()+1;var date=1;var hh=now.getHours();var mm=now.getMinutes();var ss=now.getSeconds();var ampm="";while(i_format<format.length){c=format.charAt(i_format);token="";while((format.charAt(i_format)==c)&&(i_format<format.length)){token+=format.charAt(i_format++);}
if(token=="yyyy"||token=="yy"||token=="y"){if(token=="yyyy"){x=4;y=4;}
if(token=="yy"){x=2;y=2;}
if(token=="y"){x=2;y=4;}
year=_c._getInt(val,i_val,x,y);if(year==null){return 0;}
i_val+=year.length;if(year.length==2){if(year>70){year=1900+(year-0);}
else{year=2000+(year-0);}}}
else if(token=="MM"||token=="M"){month=_c._getInt(val,i_val,token.length,2);if(month==null||(month<1)||(month>12)){return 0;}
i_val+=month.length;}
else if(token=="dd"||token=="d"){date=_c._getInt(val,i_val,token.length,2);if(date==null||(date<1)||(date>31)){return 0;}
i_val+=date.length;}
else if(token=="mm"||token=="m"){mm=_c._getInt(val,i_val,token.length,2);if(mm==null||(mm<0)||(mm>59)){return 0;}
i_val+=mm.length;}
...
_c.parseDate=function(val){var preferEuro=(arguments.length==2)?arguments[1]:false;generalFormats=new Array('y-M-d','MMM d, y','MMM d,y','y-MMM-d','d-MMM-y','MMM d');monthFirst=new Array('M/d/y','M-d-y','M.d.y','MMM-d','M/d','M-d');dateFirst=new Array('d/M/y','d-M-y','d.M.y','d-MMM','d/M','d-M');var checkList=new Array('generalFormats',preferEuro?'dateFirst':'monthFirst',preferEuro?'monthFirst':'dateFirst');var d=null;for(var i=0;i<checkList.length;i++){var l=window[checkList[i]];for(var j=0;j<l.length;j++){d=_c.getDateFromFormat(val,l[j]);if(d!=0){return new Date(d);}}}
return null;}
_c.PopupWindow_showPopup=function(anchorname,inputobj){var p=objpos(inputobj);this.x=p.x+this.offsetX;this.y=p.y+this.offsetY;if(!this.populated&&(this.contents!="")){this.populated=true;this.refresh();}
if(this.divObj!=null){this.divObj.style.left=this.x+"px";this.divObj.style.top=this.y+"px";this.divObj.style.visibility="visible";_c.cal_displayed=1;}
click_observers.push(this);this.click_index=click_observers.length-1;}
_c.PopupWindow_hidePopup=function(){if(this.divObj){this.divObj.style.visibility="hidden";_c.cal_displayed=0;}
else{if(this.popupWindow&&!this.popupWindow.closed){this.popupWindow.close();this.popupWindow=null;_c.cal_displayed=0;}}
if(this.click_index!=-1){delete click_observers[this.click_index];this.click_index=-1;}}
_c.PopupWindow=function(parent){this.index=_c.popup_list.length;_c.popup_list[_c.popup_list.length]=this;this.divObj=parent;this.popupWindow=null;this.width=0;this.height=0;this.populated=false;this.visible=false;this.contents="";this.url="";this.type="DIV";this.use_byId=true;this.use_css=false;this.use_layers=false;this.offsetX=0;this.offsetY=0;this.populate=function(contents){this.contents=contents;this.populated=false;};this.setUrl=function(url){this.url=url;}
this.refresh=function(){if(this.divObj){this.divObj.innerHTML=this.contents;this.divObj.style.visibility="visible";_c.cal_displayed=1;}}
this.showPopup=_c.PopupWindow_showPopup;this.hidePopup=_c.PopupWindow_hidePopup;this.setSize=function(width,height){this.width=width;this.height=height;}
this.hideIfNotClicked=function(e){this.hidePopup();};this.notify_click=function(e,target){if(target.className.substr(0,2)!='cp'){this.hidePopup();}}}
_c.CalendarPopup=function(){var body=document.getElementsByTagName('body')[0]
var parent=$a(body,'div','caldiv');var c=new _c.PopupWindow(parent);c.setSize(150,175);c.offsetX=0;c.offsetY=25;c.monthNames=new Array("January","February","March","April","May","June","July","August","September","October","November","December");c.monthAbbreviations=new Array("Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec");c.dayHeaders=new Array("S","M","T","W","T","F","S");c.returnFunction="_c.CP_tmpReturnFunction";c.returnMonthFunction="_c.CP_tmpReturnMonthFunction";c.returnQuarterFunction="_c.CP_tmpReturnQuarterFunction";c.returnYearFunction="_c.CP_tmpReturnYearFunction";c.weekStartDay=0;c.isShowYearNavigation=false;c.displayType="date";c.disabledDatesExpression="";c.yearSelectStartOffset=2;c.currentDate=null;c.todayText="Today";c.cssPrefix="";window._c.CP_calendarObject=null;window._c.CP_targetInput=null;window._c.CP_dateFormat="MM/dd/yyyy";c.copyMonthNamesToWindow=_c.CP_copyMonthNamesToWindow;c.setReturnFunction=_c.CP_setReturnFunction;c.setReturnMonthFunction=_c.CP_setReturnMonthFunction;c.setReturnQuarterFunction=_c.CP_setReturnQuarterFunction;c.setReturnYearFunction=_c.CP_setReturnYearFunction;c.setMonthNames=_c.CP_setMonthNames;c.setMonthAbbreviations=_c.CP_setMonthAbbreviations;c.setDayHeaders=_c.CP_setDayHeaders;c.setWeekStartDay=_c.CP_setWeekStartDay;c.setDisplayType=_c.CP_setDisplayType;c.setYearSelectStartOffset=_c.CP_setYearSelectStartOffset;c.setTodayText=_c.CP_setTodayText;c.showYearNavigation=_c.CP_showYearNavigation;c.showCalendar=_c.CP_showCalendar;c.hideCalendar=_c.CP_hideCalendar;c.refreshCalendar=_c.CP_refreshCalendar;c.getCalendar=_c.CP_getCalendar;c.select=_c.CP_select;c.setCssPrefix=_c.CP_setCssPrefix;c.copyMonthNamesToWindow();return c;}
_c.CP_copyMonthNamesToWindow=function(){if(typeof(window._c.MONTH_NAMES)!="undefined"&&window._c.MONTH_NAMES!=null){window._c.MONTH_NAMES=new Array();for(var i=0;i<this.monthNames.length;i++){window._c.MONTH_NAMES[window._c.MONTH_NAMES.length]=this.monthNames[i];}
for(var i=0;i<this.monthAbbreviations.length;i++){window._c.MONTH_NAMES[window._c.MONTH_NAMES.length]=this.monthAbbreviations[i];}}}
var cal_clicked=0;_c.CP_tmpReturnFunction=function(y,m,d){cal_clicked=1;if(window._c.CP_targetInput!=null){var dt=new Date(y,m-1,d,0,0,0);if(window._c.CP_calendarObject!=null){window._c.CP_calendarObject.copyMonthNamesToWindow();}
window._c.CP_targetInput.value=_c.formatDate(dt,window._c.CP_dateFormat);window._c.CP_targetInput.onchange()}}
...
_c.CP_setMonthNames=function(){for(var i=0;i<arguments.length;i++){this.monthNames[i]=arguments[i];}
this.copyMonthNamesToWindow();}
_c.CP_setMonthAbbreviations=function(){for(var i=0;i<arguments.length;i++){this.monthAbbreviations[i]=arguments[i];}
this.copyMonthNamesToWindow();}
_c.CP_setDayHeaders=function(){for(var i=0;i<arguments.length;i++){this.dayHeaders[i]=arguments[i];}}
...
_c.CP_setDisplayType=function(type){if(type!="date"&&type!="week-end"&&type!="month"&&type!="quarter"&&type!="year"){alert("Invalid display type! Must be one of: date,week-end,month,quarter,year");return false;}
...
_c.CP_hideCalendar=function(){if(arguments.length>0){window._c.popup_list[arguments[0]].hidePopup();}
else{this.hidePopup();}}
_c.CP_refreshCalendar=function(index){var calObject=window._c.popup_list[index];if(arguments.length>1){calObject.populate(calObject.getCalendar(arguments[1],arguments[2],arguments[3],arguments[4],arguments[5]));}
else{calObject.populate(calObject.getCalendar());}
calObject.refresh();}
_c.CP_showCalendar=function(anchorname,inputobj){this.populate(this.getCalendar());this.showPopup(anchorname,inputobj);}
_c.CP_select=function(inputobj,linkname,format){var selectedDate=(arguments.length>3)?arguments[3]:null;if(inputobj.disabled){return;}
window._c.CP_targetInput=inputobj;window._c.CP_calendarObject=this;this.currentDate=null;var time=0;if(selectedDate!=null){time=_c.getDateFromFormat(selectedDate,format)}
...
window._c.CP_dateFormat=format;this.showCalendar(linkname,inputobj);}
_c.CP_getCalendar=function(){var now=new Date();var windowref="";var result="";result+='<TABLE CLASS="cpBorder" WIDTH=144 BORDER=1 BORDERWIDTH=1 CELLSPACING=0 CELLPADDING=1>\n';result+='<TR><TD ALIGN=CENTER><CENTER>\n';if(this.displayType=="date"||this.displayType=="week-end"){if(this.currentDate==null){this.currentDate=now;}
...
this.init();this.oP=param?param:{};var k,def={minchars:1,meth:"get",varname:"input",className:"autosuggest",timeout:2000,delay:1000,offsety:-5,shownoresults:true,noresults:"No results!",maxheight:250,cache:true,maxentries:25,fixed_options:false,xdelta:0,ydelta:5}
for(k in def)
...
window.clearTimeout(this.clear_timer);var st=this.iHigh;if(!same_key)st--;for(var i=st;i<this.aSug.length;i++){if(this.aSug[i].value.substr(0,this.user_inp.length).toLowerCase()==this.user_inp){this.setHighlight(i+1,true);this.resetTimeout();return;}}
this.clear_timer=window.setTimeout('if(cur_autosug)cur_autosug.clear_user_inp()',500);for(var i=0;i<st;i++){if(this.aSug[i].value.substr(0,this.user_inp.length).toLowerCase()==this.user_inp){this.setHighlight(i+1,true);this.resetTimeout();return;}}}
...
if(this.ul)
delete this.ul;this.iHigh=0;if(this.oP.fixed_options&&this.fld.field_object&&this.fld.field_object.docname){var d=locals[this.fld.field_object.doctype][this.fld.field_object.docname];if(this.fld.fieldname){this.fld.value=d[this.fld.fieldname];}}};$ce=function(type,attr,cont,html)
...
var datepicker_active=0;function DateField(){}DateField.prototype=new Field();DateField.prototype.with_label=1;DateField.prototype.make_input=function(){var me=this;this.user_fmt=locals['Control Panel']['Control Panel'].date_format;if(!this.user_fmt)this.user_fmt='dd-mm-yy';makeinput_popup(this,'images/icons/calendar.gif');me.show_cal=function(cal){if(cal)_c.cal=cal;var user_fmt=me.user_fmt.replace('mm','MM');_c.cal.select(me.txt,me.txt.getAttribute('id'),user_fmt);if(isIE&&window.event){window.event.cancelBubble=true;window.event.returnValue=false;}}
me.btn.onclick=function(){if(_c.cal_displayed){_c.cal.hidePopup();}else{hide_selects();new_widget('_c.CalendarPopup',me.show_cal,1);}}
me.txt.onchange=function(){if(this.value==null)this.value='';me.set(dateutil.str_to_user(me.input.value));me.run_trigger();}
...
return v;};var _last_link_value=null;function LinkField(){}LinkField.prototype=new Field();LinkField.prototype.with_label=1;LinkField.prototype.make_input=function(){makeinput_popup(this,'images/icons/magnifier.gif','images/icons/arrow_right.gif');var me=this;me.btn.onclick=function(){selector.set(me,me.df.options,me.df.label);selector.show(me.txt);}
if(me.btn1)me.btn1.onclick=function(){if(me.txt.value&&me.df.options){loaddoc(me.df.options,me.txt.value);}}
me.txt.onchange=function(){if(_last_link_value)
return
if(me.as&&me.as.ul){}else{me.set(me.txt.value);_last_link_value=me.txt.value
...
if(this.df.fieldtype=='Text Editor'){code_editors[me.df.fieldname]=me;if(!tinymce_loaded){tinymce_loaded=1;tinyMCE_GZ.init({themes:"advanced",plugins:"style,table,indicime",languages:"en",disk_cache:true},function(){me.setup_editor()});}else{this.setup_editor();}}else{$y(me.input,{fontFamily:'Courier, Fixed'});}}
...
CodeField.prototype.setup_editor=function(){var me=this;tinyMCE.init({theme:"advanced",mode:"none",plugins:"table,style,indicime",theme_advanced_toolbar_location:"top",theme_advanced_toolbar_align:"left",theme_advanced_statusbar_location:"bottom",extended_valid_elements:"div[id|dir|class|align|style]",width:'100%',height:'360px',theme_advanced_buttons1:"save,newdocument,|,bold,italic,underline,strikethrough,|,justifyleft,justifycenter,justifyright,justifyfull,|,styleselect,formatselect,fontselect,fontsizeselect",theme_advanced_buttons2:"cut,copy,paste,pastetext,pasteword,|,search,replace,|,bullist,numlist,|,outdent,indent,blockquote,|,undo,redo,|,link,unlink,cleanup,help,code,|,forecolor,backcolor,|,indicime,indicimehelp",theme_advanced_buttons3:"tablecontrols,styleprops,|,hr,removeformat,visualaid,|,sub,sup",init_instance_callback:"code_editors."+this.df.fieldname+".editor_init_callback",onchange_callback:"code_editors."+this.df.fieldname+".input.onchange"});this.editor_init_callback=function(){if(cur_frm)
...
function SelectField(){}SelectField.prototype=new Field();SelectField.prototype.with_label=1;SelectField.prototype.make_input=function(){var me=this;var opt=[];if(this.not_in_form&&(!this.df.single_select)){this.input=$a(this.input_area,'select');this.input.multiple=true;this.input.style.height='4em';var lab=$a(this.input_area,'div',{fontSize:'9px',color:'#999'});lab.innerHTML='(Use Ctrl+Click to select multiple or de-select)'}else{this.input=new SelectWidget(this.input_area,[],'80%');$y(this.input.wrapper,{marginLeft:'1px'});this.txt=this.input.inp;this.btn=this.input.btn;this.txt.field_object=this;this.txt.fieldname=this.df.fieldname;this.txt.onchange=function(){if(me.validate)
...
var about_dialog;function WNToolbar(parent){var me=this;this.setup=function(){this.wrapper=$a(parent,'div','',{borderBottom:'1px solid #CCC',paddingLeft:'24px',background:'url("images/logos/wnf24.gif") left no-repeat',backgroundPosition:'4px 2px',backgroundColor:'#EEEEEE'});this.body_tab=make_table(this.wrapper,1,3,'100%',['30%','45%','25%'],{padding:'2px'});$y($td(this.body_tab,0,1),{paddingTop:'3px',paddingBottom:'0px'});this.model_tab=make_table($td(this.body_tab,0,1),1,4,null,['140px','140px','140px'],{padding:'2px'});this.menu=new MenuToolbar($td(this.body_tab,0,0));this.setup_home();this.setup_recent();this.setup_options();this.setup_help();this.setup_new();this.setup_report_builder();this.setup_logout();this.setup_search();}
...
if(t[0]=='Form'){_history_current=newLocation;loaddoc(t[1],t[2]);}else if(t[0]=='Report'){_history_current=newLocation;loadreport(t[1],t[2]);}else if(t[0]=='Page'){_history_current=newLocation;loadpage(t[1]);}else if(t[0]=='Application'){_history_current=newLocation;loadapp(t[1]);}};search_fields={};function setlinkvalue(name){selector.hide();selector.input.set(name);selector.input.set_input(name);if(selector.input.txt)selector.input.txt.onchange();}
...
d.sel_type=dt;d.set_title('Quick Search for '+dt);}
...
var p=new Page(page_name,pdoc.__content?pdoc.__content:pdoc.content);var script=pdoc.__script?pdoc.__script:pdoc.script;p.doc=pdoc;if(script)
...
local_name_idx={};LocalDB.get_localname=function(doctype){if(!local_name_idx[doctype])local_name_idx[doctype]=1;var n='New '+doctype+' '+local_name_idx[doctype];local_name_idx[doctype]++;return n;}
...
function Body(){this.left_sidebar=null;this.right_sidebar=null;var me=this;this.no_of_columns=function(){var n=1;if(cint(this.cp.left_sidebar_width))n++;if(cint(this.cp.right_sidebar_width))n++;return n;}
...
c++;}}}
this.setup_sidebar_menu=function(){if(this.left_sidebar){new_widget('SidebarMenu',function(m){sidebar_menu=m;m.make_menu('');});}}
...
this.setup();}
var popup_cont;function startup(){dhtmlHistory.initialize();dhtmlHistory.addListener(historyChange);popup_cont=$a(document.getElementsByTagName('body')[0],'div');var setup_globals=function(r){profile=r.profile;user=r.profile.name;user_fullname=profile.first_name+(r.profile.last_name?(' '+r.profile.last_name):'');user_defaults=profile.defaults;user_roles=profile.roles;user_email=profile.email;profile.start_items=r.start_items;account_name=r.account_name;home_page=r.home_page;sys_defaults=r.sysdefaults;}
...
var callback=function(r,rt){if(r.exc)msgprint(r.ext);setup_globals(r);setup_history(r);setup_events();page_body=new Body();page_body.run_startup_code();page_body.setup_sidebar_menu();for(var i=0;i<startup_list.length;i++){startup_list[i]();}
...
startup_list.push(setup_calendar);window.onload=function(){startup()}

...
function DateField(){}DateField.prototype=new Field();DateField.prototype.with_label=1;DateField.prototype.make_input=function(){this.user_fmt=locals['Control Panel']['Control Panel'].date_format;if(!this.user_fmt)this.user_fmt='dd-mm-yyyy';makeinput_popup(this,'images/icons/calendar.gif');var me=this;me.show_cal=function(cal){if(cal)_c.cal=cal;_c.cal.select(me.txt,me.txt.getAttribute('id'),user_fmt);if(isIE&&window.event){window.event.cancelBubble=true;window.event.returnValue=false;}}
me.btn.onclick=function(){if(_c.cal_displayed){_c.cal.hidePopup();}else{hide_selects();var user_fmt=me.user_fmt.replace('mm','MM');new_widget('_c.CalendarPopup',me.show_cal,1);}}
...
var about_dialog;function WNToolbar(parent){var me=this;this.setup=function(){this.wrapper=$a(parent,'div','',{borderBottom:'1px solid #CCC',paddingLeft:'24px',background:'url("images/logos/wnf24.gif") center left no-repeat',backgroundColor:'#EEE'});this.body_tab=make_table(this.wrapper,1,3,'100%',['30%','45%','25%'],{padding:'2px'});$y($td(this.body_tab,0,1),{paddingTop:'3px',paddingBottom:'0px'});this.model_tab=make_table($td(this.body_tab,0,1),1,4,null,['140px','140px','140px'],{padding:'2px'});this.menu=new MenuToolbar($td(this.body_tab,0,0));this.setup_home();this.setup_recent();this.setup_options();this.setup_help();this.setup_new();this.setup_report_builder();this.setup_logout();this.setup_search();}
...
var pages=[];function Page(page_name,content){var me=this;this.name=page_name;this.onshow=function(){set_title(me.name);try{if(pscript['onshow_'+me.name])pscript['onshow_'+me.name]();}catch(e){submit_error(e);}}
...
var p=new Page(page_name,pdoc.__content?pdoc.__content:pdoc.content);var script=pdoc.__script?pdoc.__script:pdoc.script;if(script)
